...
- [ ] **BIA-clone drum voice**. Clean-room from public technical manual (manuals.noiseengineering.us/bia/). Six tonal + one noise oscillator in three configurations (Skin / Liquid / Metal); Skin = 6-osc additive, Liquid = additive + pitch envelope, Metal = pair of 3-op PMM. Threshold-reflection folder with dynamic stage multiplication. Not the same lineage as Ngoma (which is analog-808/909 in feel) -- BIA-clone is digital-aliasing-friendly by contrast. Reuses jf::four::Voice from JF Phase 3a (NEON 4-lane pattern), Helicase polynomial sine, Varishape morph. **Detailed phase plan, architecture-from-manual quotes, and habitat reuse map in `planning/bia-clone-scoping.md`** (drafted 2026-05-01). Pick a generic name (Tympani / Saliens / Drupa candidates) per `feedback_no_third_party_branding` before starting Phase 1. Ships in spreadsheet alongside Ngoma + JF.
