...
- [ ] **Retrofit: hybrid float conversion on kWoodRoom + WoodenBox.** CreamCoat's 14% stereo result vs WoodenBox's 14% stereo (same FDN structure, more DSP work) implies the double-precision baseline costs roughly 2× on Cortex-A8 (no DP NEON, scalar VFPv3 fallback for every double op). Mechanical retrofit per the hybrid pattern: state arrays + per-sample intermediates + feedback taps → float; bezier accumulator + block-rate scalars stay double. Projected: **kWoodRoom 29% → ~12-15% stereo**; **WoodenBox 14% → ~6-8% stereo**. Defer until convenient (after current port pipeline lands) but high-priority backlog — substantially extends the headroom for stacking multiple house atoms in one patch.
...
Snapshot at `house-0.1.0.8` (2026-06-04). Six AW-port atoms shipped, hardware-validated: kWoodRoom (29% double) · WoodenBox (14% double) · CreamCoat (14% hybrid float) · BrightAmbience3 (sparse-prime-tap, hybrid float memory-necessary) · Verbity (15-20% hybrid float) · Galactic (25-30% hybrid float). All under MIT, all faithful ports with AW upstream names per `feedback_no_third_party_branding` open-source exception. Two outstanding backlogs:

- Retrofit kWoodRoom + WoodenBox to hybrid float (projected ~half CPU each)
- Original-design queue (RotCoat preferred first, then XYZ engine)

Also worth considering before pivoting away: **PurestConsole3 pair** (channel + buss) as the first reverb-composition building block — implements handoff §2 mechanic #1 (containment / feedback governor). Lets any house reverb run inside a level-dependent anti-runaway encode→decode loop. ~100 lines, trivial CPU. New compositional axis, not just another verb.

### Original-design reverbs (post-AW-pipeline)

- [ ] **RotCoat** — quantized-divisor verb + tape-rot per delay line. Fusion of CreamCoat's reduced-rate domain (taken to its extreme) and ChromeOxide's tape-rot character (applied inside each FDN line). Headline knob: World (stepped ÷1..÷8 divisor). Per handoff: **structurally AM335x-friendly by construction** — the reduced-rate domain cuts buffer length AND read rate by the divisor simultaneously. The stronger pick between the two originals for first implementation. Design doc: `planning/rotcoat-design.md`. Package: `house`.
