...
### Alembic
- [x] Bolt on the dual FM filter matrix from the existing Som unit. *(Done 5d-2 + 5d-3, 2.5.5.158-160. 2× TPT SVF in series with topoMix/bpLpBlend/shared Q/drive + 10-source × 6-destination × 8-lane mux routing matrix, hard-cut per-slot.)*
- [x] SOM filter pair mapping options. *(Done 5d-2 / 5d-2.1 / 5d-3. Trained per-node: cutoff1=brightness, cutoff2=(entropy+flux)/2, Q=1-flatness, topoMix=runLen, bpLpBlend=pitched, drive=noisy*0.6+flux*0.4, plus 8 routing lanes via feature-driven (src,dst,atten) selection.)*
- [ ] Phase 6: serialization. Sample reference + per-node trained state (43 floats × 64 + uint8 lane indices + wavetable LUT) + user-bias adapter biases. Deferred until Phase 7 parameters land so the persisted shape is final on first ship. Quicksave round-trip must survive sample-pool absence (silently revert to placeholder).
- [ ] Phase 7: group fades + Ferment/Crucible/Reagent globals. Re-expose the 36 hidden user-bias C++ Parameters (ratios×4, levels×4, detunes×4, matrix×16, filter base×6, reagent, plus already-visible scan/reagentScan) as group fades that crossfade trained per-node values vs. user-direct values. Candidate fades: wPitch (ratios+detunes), wDynamics (levels), wStructure (matrix off-diag), wFeedback (matrix diag), wFilter (filter base), wRouting (lane attens). Globals: Ferment (post-blend off-diag scalar), Crucible (post-blend diagonal scalar), Reagent global (already wired in 5d-1.5 as amount multiplier). Ply layout requires its own design pass -- 9+ new parameters won't fit a single expanded view, likely splits across two named views (play/shape) per planning/alchemy-voice.md ply section.
- [ ] Phase 8: Order 2/3 derivations + sample-pointer excitation + meta-mapping. Order 2 = gradients across map (this node vs K scan-neighbors → detune offsets). Order 3 = topology metrics (eccentricity / sparsity → reagent flag). Sample-pointer excitation = voice reads the trained sample as a node-offset modulator. Plus the meta-mapping targets below: (a) per-sample feature variance bounds the per-node chaosScore range; (b) mean entropy across picks biases topology distribution; (c) sample length adjusts coarse hop / pick density; (d) stereo correlation drives detune asymmetry; (e) DC offset / waveform asymmetry drives matrix-path bias direction. Add each "earns its complexity" -- compare audibly against Phase 7 baseline; drop any that don't lift the unit.
- [ ] Phase 9: polish + naming. Final pass on names, mnemonic, defaults, sphere viz refinements, control labels.
- [ ] Serial stacked waveform viz for the 64 trained wavetable frames (Phase 5d artifact). Render `mWavetableLUT[64][256]` as a stacked / cascading waveform display — perspective stack down the Z axis with the active reagentScan-position frames lit, neighbors dimmed, time-sorted ordering visible at a glance. Could supplement the sphere viz (sphere = main scan / PM voice, waveform stack = reagent scan / shaper) or live as a per-unit "wavetable inspector" view.
