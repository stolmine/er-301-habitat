...
- [ ] Sub-display selection indicator lands on the wrong sub-button after ply-to-ply navigation (2026-04-21, Helicase). **Repro:** focus sub N on top-level control 1 (e.g. sub1 = mixReadout on HelicaseOverviewControl). Press M key to move cursor to top-level control 2 (HelicaseModControl). On control 2, encoder correctly edits control 2's sub N -- sub-index carries over as desired. But the sub-display highlight on control 2 is drawn under sub3, not sub N. **Root cause identified (user, 2026-04-21):** inherited from stock GainBias behavior -- GainBias auto-focuses `self.bias` on top-level control focus change, and `self.bias` lives at sub3 position in the stock sub-display. Our Pattern A paramSubGraphic overrides the sub-buttons for that position but the framework-level encoder-focus indicator still lands where the base GainBias expects the bias readout -- i.e. sub3 -- so the user sees a spurious sub3 highlight on entry. **Fix sketch:** in Pattern A `onCursorEnter`, when `self.paramMode` is active, explicitly clear the framework-side encoder focus after delegating to GainBias (e.g. call `self:setSubCursorController(nil)` right after `GainBias.onCursorEnter`) so the inherited bias-position highlight doesn't leak into paramMode's custom sub-display. Verify that this doesn't break the case where the user tapped a sub earlier in the same session (paramFocusedReadout ~= nil across leave is cleared by leave, so this should be safe per Decision 7). Apply to every Pattern A and Pattern C control; one small onCursorEnter edit per file. Single-commit fix once validated.
