...
- [x] Zipper noise on V/Oct and comb size changes. *(2.5.5.184 -> .188. Solved across multiple iterations: Path B (`.184`, LinearRamp + fade buffer) and Path C (`.186/.187`, one-pole LP) both crashed under V/Oct + size double-modulation -- 1-2s and 20-30s respectively. Same root cause: the per-sample baseDelay slew exposed a long-latent ulp edge case in the multitap wrap math. Fix landed `.188`: float-precision-edge guard on `idx0` matching the existing `idx1` guard. See `feedback_multitap_idx_wrap_ulp.md` and `feedback_doppler_basedelay_smoother.md` for the detailed pattern. Final shape: Path C smoother (single-pipeline one-pole LP toward target baseDelay, ~25ms time constant, no LinearRamp / fade-buffer / AudioThread::getFrame overhead) + idx0 wrap guard + isfinite() reset on Internal IIR state at block top.)*
- [ ] Doppler slew time as a sub-param on the size control. Currently the LinearRamp length is hard-coded to 25 ms (`globalConfig.frameRate * 0.025f`). Expose as a user knob so the trade between zipper-clean / pitch-glide-fast and zipper-clean / pitch-glide-slow is musical: short slew = snappy chord changes with brief Doppler chirp, long slew = sweeping tape character. Implementation sketch: new `mSizeSlew` Parameter (range ~5..200 ms, default 25 ms, log map), per-block `mFade.setLength((int)(globalConfig.frameRate * slewSec))` (LinearRamp.setLength is cheap -- just updates `mLength` + `mStepUnit` if it changed). Surface as the shift sub-param on the existing combSize control (paramMode 2-column or single-sub layout, see AlembicScanControl pattern post-Phase-8e for reference).
...
- [x] **Per-band sub-param modulation moved the graphic but not audio** (same bug + fix as Parfait). The C++ read each per-band ParameterAdapter's static `"Bias"` via `setBandBias`; CV only moves `"Out"`. Repointed all 6 per-band READ refs (threshold/ratio/speed/attack/release/weight) to `"Out"`; `setBandLevelBias` stays `"Bias"` (graphic brightness only). spreadsheet 2.8.1.11. (`feedback_subparam_out_vs_bias_modulation`)
...
### UI / framework
- [ ] **Edit-mode border vanishes on expanded-graphic + M-hold (user report, 2026-05-17, needs repro).** User wording: "if you expand the graphic then hold any of the M buttons to get into Edit the border around the rectangle w/ the graphic disappears." Steps to attempt repro: (1) expand any unit's custom graphic into expansion view (M1 hold or equivalent); (2) hold any of M1-M6 to enter Edit mode; (3) observe the border drawn around the rectangle that contains the expanded graphic — reportedly it disappears. Suspected scope: framework-level (the border is drawn by the firmware around expansion graphics in Edit mode), not unit-specific. Candidate units to test: any with substantial custom graphics in expansion (Visadhara Corona, Network listener-sphere, Helicase phase-space, Filterbank spectrum, Ngoma cube, Alembic sphere, JF gates page if applicable). If repro confirms it's framework-level, file upstream against the firmware (stolmine); if it's specific to a particular Graphic subclass, fix in the package. Start by repro'ing in emu — if visible there too, instrument the Graphic::draw or the surrounding edit-mode chrome path.
...
- [ ] Random step selection mode. Instead of advancing `mStep` linearly through 1..wrapLen on each clock edge, pick the next step uniformly at random from the active range. Surface as a step-advance Option (Linear / Random / maybe Brownian as a third). Random mode breaks repeating effect sequences into kaleidoscopic mutations of the same step program -- complements the existing xform gate (which mutates the step DATA) by mutating the step ORDER instead. **Cross-pollination candidates**: Excel (TrackerSeq), Ballot (GateSeq), Etcher segments — all have a linear playhead that could benefit from a non-linear advance option. Each unit's sequencer-advance section is similar enough that the Option + random-LCG-pick pattern should port cleanly. Audit which units' downstream consumers depend on monotonic mStep (e.g. anything that reads "next step" ahead of time) before rolling out broadly.
...
- [x] True stereo operation (spreadsheet 2.7.1, shipped v2.5.1). Internal-stereo C++ Object pattern rather than dual-instance: shared sequencer / step program / CPR detector / shuffle pick / viz, paired per-channel buffers + SVF state + pitch phase + crossfade. CPR detector linked via max-of-|L|,|R| → same gain reduction to both channels (no stereo-image smear under heavy comp). FX_SHUFFLE per-loop random pick lifted out of processEffect into the sequencer-advance section so L and R always play matching slices. xform randomization stays coherent because step data is shared in one instance. Lua wiring uses the standard conditional R-channel pattern. Plan: planning/larets-stereo.md.

### Parfait (MultibandSaturator)
- [x] **Per-band sub-param modulation moved the graphic but not audio.** The C++ read each per-band ParameterAdapter's static `"Bias"` via `setBandBias`; CV only moves the adapter's `"Out"`. Repointed every per-band READ ref (amount/bias/type/weight/filterFreq/filterMorph/filterQ) to `"Out"` (= Bias when unpatched, modulated when patched). spreadsheet 2.8.1.7. **Same bug + fix in Impasto.** (`feedback_subparam_out_vs_bias_modulation`)
- [x] **Saturation-type selector too sensitive** (one detent jumped multiple shaper types). Root cause: scaling the dial-map step fights encoder acceleration. Added an opt-in discrete-encoder mode to `ModeSelector` (accumulate raw encoder counts, step ±1 per threshold — the firmware idiom, `xroot/Unit/Editor.lua` ItemHeader / `Env.EncoderThreshold`). Type uses threshold 16 (~4 detents/type; hardware eQEP is 4× quadrature). spreadsheet 2.8.1.11. Rollout candidates for other discrete selectors logged under Controls Audit.
- [-] **Distortion becomes audible as mix is brought down at high drive** (user report 2026-06-15, drive 5.2, all three bands at unity, no morph, no comp/output offsets, type=Tube/Diode/Tri Fold all reproduce). **(b) RESOLVED (shipped 2.8.1):** the discontinuous safety clamp is now the smooth `pseudoSaturate15` (`MultibandSaturator.cpp:766`). **(a) still open** — re-test whether distortion persists at low mix now that (b) shipped; if so, per-band shaper oversampling is the remaining fix. User initially thought "internal hard clipping when being mixed" but the mix line itself (`MultibandSaturator.cpp:791`, `out = dry + (wet - dry) * mix`) is a clean lerp — at mix=1 the heavily-saturated wet masks artifacts, at mix<1 the dry pulls overall level down and unmasks them. Two likely culprits, both in the wet path:
  - **(a) No oversampling on the per-band saturators** (`applyShaper`, lines 101-173). Tube (line 113-115, two `fast_tanh` branches), Diode (line 120, rational sat), Tri Fold (lines 125-132, 3 iterated reflect-folds w/ growth), Sine Fold (line 153), Fractal (lines 162-167, iterated `s - s³/3`). At drive 5.2 the effective input gain into the shaper is `1 + amount*5 = ~27×` (line 106), so all shapers will generate harmonics well above Nyquist that fold back as inharmonic content. Audible as "broken" or "aliased" rather than "more saturated." Fix: wrap `applyShaper` in 2× OS (linear upsample / shape / halfband decimate), or at least the heavier shapers (Tri Fold, Fractal). 4× preferred for Fractal since its iterated polynomial multiplies bandwidth aggressively.
  - **(b) Discontinuous safety clamp — DONE (shipped 2.8.1).** Was `if (wet > 1.5f || wet < -1.5f) wet = fast_tanh(wet * 0.67f) * 1.5f;` (derivative discontinuity at ±1.5 → aliasing harmonics). Now replaced with the always-on smooth `pseudoSaturate15(wet)` (`MultibandSaturator.cpp:766`, helper at :54) — the `x / (1 + (|x|/K)^4)^(1/4)` pseudo-saturate exactly as proposed.
  - **Audio sample**: `~/Downloads/IMG_9928.mov` (28s, drive 5.2). Spectral analysis confirms broadband content with normal music crest factors — the artifact is character not level, consistent with aliasing rather than peak clipping.
  - **Quick triage before committing oversampling work**: ship just (b) first (smooth clamp). If user still hears distortion at lower mix, that confirms (a) is the dominant cause and OS is required. The fix in (b) is cheap and removes one definite source.
  - **Tie-in**: this is exactly the "intentional aliasing" mechanic from `project_alias_synthesis_paradigm` parking-lot note, but as an *unintentional* artifact. The fix is what Mirror would deliberately defeat. Worth referencing both directions.
...
Current state: **2.5.5.164** (Phase 5d-4 complete). Memory: `~/.claude/projects/-home-sure-repos-er-301-habitat/memory/project_alembic_codex.md`. Design doc: `planning/alchemy-voice.md`.

Done in this session:
- [x] Bolt on the dual FM filter matrix from the existing Som unit. *(5d-2 + 5d-3, 2.5.5.158-160. 2× TPT SVF in series with topoMix/bpLpBlend/shared Q/drive + 10-source × 6-destination × 8-lane mux routing matrix, hard-cut per-slot.)*
- [x] SOM filter pair mapping options. *(5d-2 / 5d-2.1 / 5d-3. Trained: cutoff1=brightness, cutoff2=(entropy+flux)/2, Q=1-flatness, topoMix=runLen, bpLpBlend=pitched, drive=noisy*0.6+flux*0.4, plus 8 routing lanes via feature-driven (src,dst,atten) selection.)*
- [x] Comb filter (Pecto identical-engine clone). *(5d-4, 2.5.5.164. 24-tap multitap + 16 patterns + 4 slopes + 4 resonator types + NEON 3-pass tap gather + 20Hz DC-blocker. 6 trained slots row[43..48]: density, pitch, pattern, resType, feedback, slope. Independent comb-scan ply with single fader collapsing dry/wet + scan position.)*
- [x] Ferment chaos scalar macro. *(5d-4, 2.5.5.164. Single-axis fader, default 1.0, range [0, 1.5]. Multiplies trained matrix off-diag/diag + routing lane attens. At 0 = clean tonal voice; 1 = trained chaos; 1.5 = boost. Crucible dropped.)*
- [x] Output limiter at end of chain. *(2.5.5.163. Soft-knee asymptote toward ±1, linear up to ±0.9.)*

Remaining:
- [ ] **Phase 6: serialization.** Sample reference + per-node trained state (49 floats × 64 + uint8 lane indices + wavetable LUT[64][256]) + user-bias adapter biases. Deferred until Phase 7 parameters settle so the persisted shape is final on first ship. Quicksave round-trip must survive sample-pool absence (silently revert to placeholder).
- [ ] **Phase 7: per-region user-bias sub-params** (if needed; Ferment-as-master is the v1 control surface). Re-expose the 36 hidden user-bias C++ Parameters as fine-grained fades. Live in expanded view 2 / sub-displays — top-level ply count stays at 8.
- [ ] **Phase 8: Order 2/3 derivations + sample-pointer excitation + meta-mapping.** Order 2 = gradients across map (this node vs K scan-neighbors → detune offsets). Order 3 = topology metrics (eccentricity / sparsity → reagent flag). Sample-pointer excitation = voice reads the trained sample as a node-offset modulator. Meta-mapping: (a) per-sample feature variance bounds per-node chaosScore range; (b) mean entropy biases topology distribution; (c) sample length adjusts coarse hop; (d) stereo correlation drives detune asymmetry; (e) DC offset / waveform asymmetry drives matrix-path bias direction. Add each "earns its complexity."
- [ ] **Phase 9: polish + naming.** Final pass on names, mnemonic, defaults, sphere viz refinements, control labels.
- [ ] Serial stacked waveform viz for the 64 trained wavetable frames. Render `mWavetableLUT[64][256]` as a stacked / cascading waveform display — perspective stack with the active reagent-scan-position frames lit, neighbors dimmed, time-sorted ordering visible at a glance. Supplement the sphere viz (sphere = main scan; waveform stack = reagent scan), or per-unit "wavetable inspector" view.
- [ ] Direct sample swap (no detach first) doesn't retrain properly. Detach-then-attach via menu works. Suspected analyzeSample race against Sample.Pool async load. Fixes to try: defer analyzeSample until `Sample::isFullyLoaded()` check (if exists); or post analyzeSample to a deferred task that re-checks readiness. Reproduce: Card → sample A → load. Card → sample B without detaching → scan flat.
- [ ] **Comb retrofit: Doppler smoother + idx wrap ulp guard.** Pecto's `.184–.188` zipper-noise solve produced a Doppler-style baseDelay smoother (`feedback_doppler_basedelay_smoother.md`) and a float-precision-edge guard on idx0 (`feedback_multitap_idx_wrap_ulp.md`). Alembic's comb is a documented Pecto-clone with identical NEON 3-pass tap gather + wrap math, currently running at block-rate cached delays. The wrap ulp bug is latent there too — rare under block-rate but reachable. Retrofit plan: (a) add per-sample one-pole LP smoother on combBaseDelay (Path C pattern from Pecto), (b) add idx0 wrap guard matching the existing idx1 guard in both NEON and scalar paths, (c) `isfinite()` reset on Internal IIR state at block top (DC-blocker, fbFilterState, sitarEnvFollower equivalents). Test the same V/Oct + comb-scan double-modulation scenario that exposed the bug in Pecto. Cross-check Petrichor (MultitapDelay) and Larets for the same template per the Pecto NEON 3-pass memory.
...
- [x] **v2.4.1 released 2026-05-03 (hotfix).** spreadsheet 2.6.0 → 2.6.1 only. Ngoma DrumCubeGraphic insert hard-fault root-cause + fix: out-of-line virtual triggered GCC key-function rule, non-COMDAT vtable drifted against firmware. Moved implementation inline. Bundled prevention: `docs/graphics-authoring-guide.md` + `tools/check-graphic-virtual-defs.sh` lint. DrumVoice ctor sinf swapped to LUT. No other package changes.
- [x] **v2.5.0 released 2026-05-16.** spreadsheet 2.6.1 → 2.7.0, mi 1.0.1 → 1.0.4, catchall 0.3.0 → 0.4.0. biome/kryos/peaks/porcelain/scope unchanged. Two new units (Visadhara PMM voice in spreadsheet; Network multi-tap glitch reverb promoted from catchall to spreadsheet with full gltch character macro). Cortex-A8 NEON build defense: `-fno-tree-vectorize` appended last in CFLAGS for am335x across every mod.mk + link-time NEON-hint lint via `tools/check-neon-hints.sh`. Plaits/Clouds engine/mode switching deterministic on hardware (was an occasional hard-fault; AAPCS belt-and-suspenders `mi_barrier_noop()` plus the flag fix). Plaits 6-op FM 2x oversampled (still noisy, not done). NEON optimization pass: Filterbank SoA SVF bank (8% stereo @ 16 bands), Petrichor SoA + multi-tap weighted FB, Impasto+Parfait SoA + fast paths, Helicase polynomial sin/tanh, Rings modal kernel tighten (mi 1.0.2). Network unit reference moved from `catchall.Network` → `spreadsheet.Network` — saved-patch re-bind needed for that unit only. JF shipped in spreadsheet binary as open secret (Phase 6 polish remains, undocumented). 204 commits since v2.4.1. Release at https://github.com/stolmine/er-301-habitat/releases/tag/v2.5.0. Notes: `planning/release-v2.5.0-{github.md,discord.md,bbcode.txt,combined-discord.md,combined-bbcode.txt}`. Shipped alongside firmware `v0.7.0-stolmine.9.2.0` (Sequencer v2: dual gates + transpose meta column, per-gate L2 grammar, quicksave v1→v2 auto-migration, "Sequencer" System Settings subheading, opt-in QuickSave-restores-transport, BPM in-takeover with S2 latch).
- [x] **v2.5.1 released 2026-05-30.** scope 1.1.0 → 1.2.1, spreadsheet 2.7.0 → 2.7.1. biome/catchall/kryos/mi/peaks/porcelain unchanged. **Scope** got user-controllable timebase (7 stops, 1x..64x via FifoProbe decimation), Y-axis gain (5 stops, 0.25x..4x), and a built-in 3-decimal voltmeter readout in the S3 sub-display. Custom header-only `ScopeGraphic` + `ScopeVoltsReadout` + `ScopeControlBox` aped from firmware MiniScope, no firmware changes. Sub-display has dotted-when-idle / solid-when-encoder-grabbed box convention; VOLT renders as bare numeric (read-only, no box). **Larets** rebuilt for true stereo via the internal-stereo C++ Object pattern (paired ch[2] state, single shared sequencer + step program + CPR detector + shuffle pick). Linked CPR via max-of-|L|,|R| → one envelope → same gain to both channels (no image smear). Wire names changed `In/Out` → `In L/In R/Out L/Out R` — saved patches using menu-inserted Larets reconnect automatically; custom patch scripts referencing inlets by name need updating. Release at https://github.com/stolmine/er-301-habitat/releases/tag/v2.5.1. Notes: `RELEASE-2.5.1.md`. Same firmware compatibility as v2.5.0 (`v0.7.0-stolmine.9.2.0`).
...
- [x] **Move Alembic from spreadsheet to catchall.** *(Move 1, commit `7947fcb`. C++ + Lua moved with `git mv`; AlembicRef retired; pffft + ShiftHelpers duplicated to catchall (still needed by spreadsheet's Multiband units); Lua require strings migrated to `catchall.libcatchall`. Pre-existing typo in catchall's Lambda.lua fixed in passing. catchall 0.2.0 -> 0.3.0; spreadsheet 2.5.5.188 -> 2.5.5.189. Hardware-verified.)*
- [x] **Consolidate Mutable Instruments ports into one package.** *(Move 2, commit `8ac31ca`. 8 packages collapsed into single `mi` v1.0.0. Layout: top-level unit wrappers + `pffft`/`arm_math`; symlinks to `eurorack/{clouds,marbles,grids,stmlib}`; real override subdirs for stolmine modifications (`elements/dsp/{dsp.h,exciter.cc,multistage_envelope.h,tube.h}` 48kHz scaling; `plaits/user_data.h` STM32 stub; `rings/dsp/{part,resonator}.{h,cc}` NEON SVF + max_resolution; `warps/drivers/debug_pin.h` empty stub). Single shared stmlib build de-duplicates the previously-per-pkg stmlib copies. INCLUDES order puts `$(MOD_DIR)` first so override headers win over eurorack canonical. SYMBOLS=TEST gates the STM32 conf includes out of cloud's drivers/debug_pin.h. Lua require strings sed'd to `mi.libmi` + `mi.<helper>`; libvar names renamed for consistency. Toc category "MI" replaces per-pkg "Mutable". Top-level Makefile PROJECTS list collapses 16->8. Patch-migration policy: option C (accept break, document in release notes). All 9 unit ctors verified via `nm libmi.so`. Hardware-verified: all 9 MI units (Plaits, Clouds, Rings, Grids, Warps, Stratos, Commotio, MarblesT, MarblesX) instantiate and function. Cross-deps (Stratos's clouds-reverb reuse, Rings's plaits.EngineSelector reuse) now intra-package and work cleanly.)*
...
- [ ] Bias indication on visualizer plies. All spreadsheet units with custom viz graphics (Ngoma cube, Helicase orbital/phase, SOM sphere, Larets overview, Colmatage overview, Rauschen phase space) should overlay a dotted horizontal line showing the primary parameter bias value, visible only while the user is actively adjusting the encoder. Reference: Impasto CompressorSpectrumGraphic and Parfait SpectrumGraphic band level indication. Needs a "show indicator" flag set by the control's encoder handler and cleared after timeout or cursor leave.
- [ ] **Repo-wide fader response audit.** Many controls move too fast, scale erratically, or don't match the feel their target parameter wants (linear knob driving an exponential-perception param, etc.). Walk every fader across biome, spreadsheet, catchall, mi, peaks, scope, kryos, porcelain and verify: (1) **dial map shape** — linear vs log/exp vs cubic — matches the param's perceptual axis (frequency = log, time = log, gain = exp or dB, mix = linear, etc.); (2) **coarse / fine steps** at each precision level feel proportional to the param's useful resolution at that point in the range (e.g. cutoff fine-step shouldn't be 1 Hz at 8 kHz); (3) **encoder velocity / acceleration** lands at the intended end of the range in a comfortable number of revolutions — too-fast controls overshoot, too-slow controls fatigue; (4) **bias range vs typical-useful sub-range** — if 90% of the useful action is in the bottom 10% of travel, the map is wrong; (5) **sub-display readout matches feel** — e.g. semitones display while internally the param is Hz wants a semitones-scaled dial map. Likely-bad first-pass candidates from memory: any frequency control on a LinearDialMap, decay-time controls in ms with linear scaling, drive/gain controls without dB shaping, anything that previously got hand-tuned `* 48` or `* 100` multipliers in Lua. Produce a per-control table (current map → recommended map → reason) before changing en masse. Coordinates with the bias / mod-input range audit below since fixing the map often interacts with fixing the gainMap.
- [ ] **Discrete-encoder rollout to other selectors.** Shipped on the Parfait shaper-type selector (spreadsheet 2.8.1.11): `ModeSelector` gained an opt-in `discrete` mode that accumulates RAW encoder counts and steps ±1 once per `discreteThreshold` ticks (resets each step → acceleration-independent), mirroring the firmware idiom (`xroot/Unit/Editor.lua` ItemHeader, `Env.EncoderThreshold`). Hardware eQEP is 4× quadrature (~4 counts/detent), so **threshold ≈ 4 × (detents per option)**; Parfait type uses 16 (~4 detents/type). **Scale threshold to option count** so a full sweep stays ~30–40 detents and stays ≥2 detents/option. Roll out to the discrete selectors below (skip continuous morphs like Canals `output`, and 2-option ones like Canals `mode` where it's marginal):
  - **spreadsheet (share the patched `ModeSelector` — just add `discrete=true` + threshold):** Pecto `patternFader` (16 opts → thr ~8), Rauschen `algo` (11 → ~12), Filterbank `scale` (N Scala scales → ~8–12), MultitapDelay vol/pan macros (11 each, via `MacroControl` subclass → ~12), Pecto `resonatorFader`/`slopeFader` (4 → ~16–20), MultitapDelay `grid` (5)/`stack` (4) (~16).
  - **other packages (need the discrete block copied into their own `ModeSelector.lua` first):** Clouds `mode` (mi), Chime `scale` (porcelain), Sfera `config` + Lambda `seed` (catchall), stolmine duplicates (Canals/Filterbank/Etcher/TrackerSeq).
  - Best as one build per package (single hardware round-trip). Per-control threshold is the only tuning.
- [ ] Bias and mod-input range audit across all packages. Several controls use wide integer bias ranges (notably frequency-class params: cutoffs, decay times in ms, pitch sweep in semitones, etc.) where the default mod-input gainMap tops out at 10x. Typical CV sources (0..1 envelopes, -1..1 LFOs, triggers at 1.0) multiplied by 10 can't cover the bias range, so mod inputs reach nothing meaningful. Two possible unifications: (a) re-scale bias to a normalized 0..1 or -1..1 surface with internal quadratic / log mapping to the underlying param (broader mod reach, changes fader feel); or (b) expand per-control gainMap defaults (e.g. `app.LinearDialMap(-max, max)` where max matches the bias range) so 1.0 CV can sweep the full param. Need to walk every Pattern A / B control + SDK-derived GainBias usage across biome, spreadsheet, catchall, MI ports, cataloging: (1) bias range, (2) typical useful sub-range, (3) current gainMap, (4) CV sources the user is likely to reach for. Produce a table + canonical-policy recommendation before touching any individual control. Frequency and time-domain params are the highest-impact targets (Rauschen cutoff, Helicase sync threshold, drum-voice sweep/decay, Pecto feedback tone, Filterbank per-band freq, etc.).

## Keyword / Picker Metadata Audit

- [ ] Walk every unit's `keywords` field across all packages and normalize. The recently-added unit picker actually consumes this field for search/sort, so units with weak, inconsistent, or missing keywords don't surface cleanly. Per-unit fixups should include (1) canonicalize casing (saw `CV` mixed with `cv`, `EQ` mixed with `eq` — pick lowercase across the board), (2) include the unit's own short name as one of the keywords so name-search works, (3) include common synonyms / aliases users might type (e.g. "reverb" alongside "verb"; "compressor" alongside "comp" alongside "limiter" where the unit does limiting too; "shaper" alongside "waveshaper" alongside "fold" for waveshaping units), (4) include the functional family (sequencer / oscillator / filter / shaper / reverb / delay / utility / measurement / etc.) so genre-style picker queries land, (5) make sure the *intended* picker category is reflected in the keyword set (so a user typing "drum" finds Ngoma, Bass Drum, Snare Drum, FM Drum, Peaks Drum, etc., not just whichever toc entry happened to include "drum"). Walk in this order: scope, biome, spreadsheet, catchall, mi (MI ports), peaks, kryos, porcelain. Cross-check against the unit picker's actual search algorithm before committing the normalization (case-sensitive substring? token match? we should know).
...
- [x] Timebase and gain controls (scope 1.2.1, shipped v2.5.1). Header-only `ScopeGraphic.h` package-side, aped from firmware MiniScope, exposes `setDecimation` (uses public `FifoProbe::setDecimation`) and `setGain` (Y-axis scale in `calculate()`). Shared `ScopeView.lua` ViewControl with sub-display Time (7 stops, 1x..64x) + Gain (5 stops, 0.25x..4x) selectors driven by M1/M2 focus + encoder stepping. All three variants (Scope, Scope 2x, Scope Stereo) share the same view; stereo applies time/gain to both L and R graphics. State persists via Unit serialize/deserialize. No firmware changes. Plan: planning/scope-timebase-gain.md. Probe reset + warmup re-enter on decimation change so the display draws homogeneous-rate samples (no bunched-up splice artifact during FIFO rollover). Restored warmup-counter increment invariant from MiniScope (don't "improve" reference control flow — see `feedback_identical_means_identical.md`).
- [ ] Research headerless unit display (SDK hardcodes header — not currently possible)
- [x] Voltmeter readout on existing scope units (scope 1.2.1, shipped v2.5.1). Sub-display S3 column shows a 3-decimal rolling-mean voltage scaled to ER-301 ±10 V range, useful for DC and V/Oct analysis. Header-only `ScopeVoltsReadout` graphic follows the scope's `ScopeGraphic` instance and reads `getVolts()` (computed from the full probe buffer mean × 10) on each draw. Integration window scales with timebase: ~0.33 s at decimation=2, ~10 s at 64. Right-justified text so the decimal column stays put as digits change. No box around the readout (it's read-only); TIME/GAIN editable controls have dotted-when-idle / solid-when-encoder-grabbed boxes via header-only `ScopeControlBox` graphic. (Original todo proposed a separate voltmeter unit — folded into the existing scope variants instead since the probe + buffer infrastructure was already there.)
...
- [x] **paramModeDefaultSub misuse across all five Ngoma controls.** Fixed 2026-04-23 in commit `d7ecfe3`. Assignments removed; unset = nil = matches Decision 8 convention for non-bias-bound controls.
- [x] Level third sub button. Filled with Makeup (+0..+6 dB post-level) per commit `79c9a13`. Label "Clipper / EQ / Mkup".
- [x] Expanded views. Commit `562a49f` converted the flat expanded list to the hierarchical per-ply dict used by Petrichor/Parfait. Added auxiliary GainBias controls (charShape/Grit/Punch, sweepTime, decayHold/Attack, levelClipper/EQ/Makeup, tuneOctave, xformDepth/Spread) binding to the same ParameterAdapters as the headline sub-displays.
- [ ] Cube viz refinement. Deferred -- user-scoped out of the v2 follow-up pass.
- [x] EQ range to -1..1. Commit `009703d`. Bipolar -1..1 with 0 bypass. Serialization migration via `schema = 2` tag in serialize output; old patches (no schema) with eq in [0,1] get remapped via `(eq - 0.5) * 2` at deserialize.
...
- [x] **Airwindows reverb port research** (2026-05-30). Full per-candidate writeup + per-bucket ranking + DON'T-PORT list in `planning/airwindows-reverb-research.md`. Headline: **Galactic** (bombastic, ~488 KB, LOW risk, mature 3-stage FDN with fixed-tap reads) → **kWoodRoom** (small wooden room, ~220 KB, MEDIUM risk, dual 3x3/6x6 nested matrix with Bezier undersampling) → **Galactic2** (pocket, ~216 KB, LOW risk, parameter respin of Galactic). All three covered are MIT, none of the top-3 contain CloudSeed-trap patterns (no modulated allpass chains, no init-order DSP, no per-sample topology switches). Surprising findings: **PocketVerbs is the LARGEST reverb in the catalog** (~4.2 MB stereo) despite the name, MV has a per-sample 26-case switch on `stage` matching the documented runtime-branched-DSP-dispatch crash pattern, VerbTiny is not actually tiny (~525 KB stereo from doubled a4/b4 banks), Pyewacket isn't a reverb at all (waveshaper). DO-NOT-PORT list: PocketVerbs, MV, MatrixVerb, Reverb (namesake), Verbity/Verbity2 (redundant with Galactic), VerbSixes/VerbThic.
- [-] **Airwindows port queue** (revised 2026-06-02 after handoff integration — see `planning/refs/airwindows-port-handoff.md` + `planning/airwindows-reverb-research.md` addendum). Active port: **kWoodRoom** (kWoodRoom). Then in order: WoodenBox → CreamCoat → BrightAmbience3 → Galactic → Verbity. All land in the new `house` package.
- [x] **Airwindows port (1): kWoodRoom.** Shipped at `house-0.1.0.4` (2026-06-04). Phase A scaffold → Phase 0 Smoketest → Phase 1 atom + unit. **Hardware-validated**: works first try, sounds "absolutely wonderful" per user audition, CPU 29% stereo on Cortex-A8 (single instance). Phases 4+ (NEON Householder reduction → projected ~12-15% stereo, float-conversion of 3×3 trellis → ~24 KB saved) deferred until a CPU regression actually bites. Plan: `planning/kwoodroom-port-plan.md`. Template captured in `feedback_aw_atom_port_template` for the next ports.
- [x] **Airwindows port (2): WoodenBox.** Shipped at `house-0.1.0.5` (2026-06-04). 4-stage 4×4 diff-Householder FDN + single Bezier-undersample; 3 params (Select 17-stop / Reso / Mix); ~53 KB / instance. **Hardware-validated**: works perfectly, Select transitions audibly smooth despite the state-reset (no fade-down mitigation needed), CPU 14% stereo on Cortex-A8. Skipped Phase 0 Smoketest per the template — kWoodRoom validated the structural pattern and WoodenBox is strictly simpler. AW source's intentional L/R swap through the verb preserved literally per `feedback_identical_means_identical`. Plan: `planning/woodenbox-port-plan.md`.
- [x] **Airwindows port (3): CreamCoat.** Shipped at `house-0.1.0.6` (2026-06-04). 4×4 FDN + user-DeRez Bezier + 15000-sample predelay; 5 params; ~284 KB stereo post-float-conversion. **First atom to ship with hybrid float-conversion as Phase 1 default**: state arrays + intermediates + feedback taps as float, bez[] accumulator + block-rate scalars stay double. **Hardware-validated**: works great, CPU 14% stereo at DeRez=1.0 — same as WoodenBox despite more DSP complexity (predelay + InL/UnInL bezier slots). Implies hybrid float delivers ~2× CPU win vs double baseline on Cortex-A8 — see retrofit candidates below. Plan: `planning/creamcoat-port-plan.md`.
- [x] **Airwindows port (4): BrightAmbience3.** Shipped at `house-0.1.0.7` (2026-06-04). Sparse-prime-tap delay summation (488 prime offsets into a 32768-sample circular buffer per side; user picks tap window via Position + Size) with resonant SVF in the feedback path. 4 params (Position / Size / Brightness / Wetness, all 0..1 default 0.5). First non-FDN atom; first to ship with hybrid float as **memory-necessary** rather than just CPU-optimization (full double pL/pR would be 512 KB stereo, blowing the 256 KB Cortex-A8 L2 cache 2x). Block-rate hoist of `cbrt(length)` saves a divide + cbrt per cycle. **Hardware-validated**: works. CPU sensitivity to Size knob noted in the plan; not yet stressed at upper range. Plan: `planning/brightambience3-port-plan.md`.
- [x] **Airwindows port (5): Verbity.** Shipped at `house-0.1.0.8` (2026-06-04). 3-stage cascaded 4×4 diff-Householder FDN per side, IIR lowpass at input AND output, per-feedback-tap interpolation smoother, sub-low "thunder" chase. 4 params (Bigness / Longness / Darkness / Wetness). Submix wet/dry (Wetness=0.5 sums full wet AND full dry, send-style). No predelay, no LFO, no cross-coupling. ~503 KB stereo at hybrid float (exceeds L2 but per-cycle working set is just 24 cache lines — standard FDN pattern). Lightest trap profile of any house atom (no transcendentals per sample, no modulated reads). Plan: `planning/verbity-port-plan.md`. **Hardware-validated**: works.
- [x] **Airwindows port (6): Galactic.** Shipped at `house-0.1.0.8` (2026-06-04). Same 3-stage 4×4 FDN as Verbity PLUS 256-sample LFO-modulated predelay AND full L↔R cross-coupling at the feedback stage (L input receives R's feedback, R receives L's — signature lush stereo wash). 5 params (Replace / Brightness / Detune / BigDim / DryWet). AW defaults preserved: BigDim=1.0 and DryWet=1.0 (max size + full wet on insert). ~528 KB stereo at hybrid float. Heaviest atom in the package: 2 sin() per sample per channel for LFO (~10% CPU floor for sin alone). LOAD-BEARING `vibM=3.0` and `oldfpd=429496.7295` first-frame initial values preserved literally — produce an immediate first-sample LFO wrap that resets oldfpd to ~0.43 for normal operation. Plan: `planning/galactic-port-plan.md`. **Hardware-validated**: works.

### House package — reverb chapter closed (six atoms shipped)

Snapshot at `house-0.1.0.8` (2026-06-04). Six AW-port atoms shipped, hardware-validated: kWoodRoom (29% double) · WoodenBox (14% double) · CreamCoat (14% hybrid float) · BrightAmbience3 (sparse-prime-tap, hybrid float memory-necessary) · Verbity (15-20% hybrid float) · Galactic (25-30% hybrid float). All under MIT, all faithful ports with AW upstream names per `feedback_no_third_party_branding` open-source exception.

**Going-forward principle** (per `feedback_atoms_as_components`): future atoms intended as composition building blocks (tape-rot, saturators, band-split, undersample shells, Console containment) ship as C++-only components — no Lua unit wrapper, no toc entry. The six shipped reverbs are units; building-block atoms are not. Promotion is mechanical and reversible-painful, so default to component-only and promote later only if standalone user value exists.

### Retrofits + RotCoat prerequisites (active backlog)

- [ ] **Retrofit (1): kWoodRoom hybrid float.** Mechanical conversion using the same hybrid pattern as CreamCoat (state arrays + per-sample intermediates + feedback taps → float; bezier accumulator + block-rate scalars stay double). Projected: **29% → ~12-15% stereo**. ER 3×3 trellis stays double-ish if precision matters; main 6×6 FDN to float. Bump PKGVERSION 4th digit. Hardware-validate.
- [ ] **Retrofit (2): WoodenBox hybrid float.** Same mechanical pattern. Projected: **14% → ~6-8% stereo**. After this, lightest house atom by 2× — multiple instances stack comfortably. Bump PKGVERSION 4th digit.
- [ ] **RotCoat prereq (1): ChromeOxide atom (component-only).** Per `planning/airwindows-primitives-inventory.md` line 71+108: ChromeOxide is explicitly the **per-line tape-rot seed for RotCoat** ("ChromeOxide (tape rot, feeds RotCoat)" in the first-15 starter port list). Dual-band tape character: lows saturated, highs delayed by noise-FM warble + bias. State: short-delay + noise. Cost: cheap. Ship as `mods/house/atoms/ChromeOxide.h` only — no Lua unit, no toc entry. RotCoat will consume it as a per-line stage. Source: `~/repos/airwindows/plugins/MacVST/ChromeOxide/source/`.
- [ ] **RotCoat prereq (2): Spiral atom (component-only).** Smoothest-curve `sin()` saturator in the AW catalog (per `planning/airwindows-primitives-inventory.md` line 27). Stateless, trivial cost. Use as the **head-bump saturation on the low band** within each RotCoat FDN line. Ship as `mods/house/atoms/Spiral.h` only. Source: `~/repos/airwindows/plugins/MacVST/Spiral/source/`. (Alternative: Density — bipolar, has highpass-just-the-distorted-path option; richer but heavier. Spiral is the minimum sufficient.)
- [ ] **RotCoat prereq (3, optional): BandSplit2 atom.** Not in AW catalog — RotCoat needs an LP/HP crossover (suggested 1.5–3 kHz) per line for the band-split. Could be 2 biquads inline in RotCoat OR a reusable atom. Recommend defer to inline — only ~10 lines, doesn't earn a separate atom unless something else also wants band-split.
- [ ] **RotCoat prereq (4, optional): BezierUndersampleShell atom.** Extract the divisor + Bezier accumulate-and-reconstruct mechanic from CreamCoat as a reusable component. Would let RotCoat and (later) XYZ share the same undersample shell. Recommend defer — the inline pattern is well-understood from CreamCoat and writing it twice is cheaper than abstracting prematurely. Lift only if a third consumer appears.
- [ ] **Optional building block: PurestConsole3 pair (channel + buss).** Handoff §2 mechanic #1 (containment / feedback governor). ~8 ops/sample/ch, trivial state, ~100 lines total. Lets any house reverb run inside a level-dependent anti-runaway encode→decode loop. New compositional axis, not just another verb. Component-only — no units. Source: `~/repos/airwindows/plugins/MacVST/PurestConsole3Channel/` + `~/repos/airwindows/plugins/MacVST/PurestConsole3Buss/`.

### Original-design reverbs (post-AW-pipeline)

- [x] **RotCoat (codename)** — SHIPPED at `house-0.1.0.14` (2026-06-05), hardware-validated. Original-design reverb. **Implementation diverged from the original ChromeOxide-per-line plan**: the shipped architecture is a multi-world per-line cycleStep fan — each of the 4 FDN lines runs at its own reduced-rate divisor, with Mulch controlling spread around the snapped World base rate. At Mulch=0 all lines share `worldRate` (classic single-rate FDN); at Mulch=1 spread factors `[1.4, 1.15, 0.85, 0.6]` fan the per-line rates. Asynchronous per-line firing; Householder cross-feeds the four worlds. Spiral saturator + per-line one-pole LP (fc≈500Hz, alpha block-rate scaled by worldRate) on feedback path = "lows recirculate" character + magnitude bound. World=1 + high-Mulch sound-design corner (aggressive crushed-aliasing) documented in atom header. Final 5-param surface: World (snapped {1,2,3,4,6,8}) / Regen / Predelay / Mulch / Wetness. **Codename `RotCoat` still in source — needs habitat-native rename** (Lath / Cure / Sediment / Patina / other) before first release. Plan: `planning/rotcoat-port-plan.md` (with shipped-status header explaining the divergence). Component dependencies: `mods/house/atoms/Spiral.h` (consumed as feedback governor), `mods/house/atoms/ChromeOxide.h` (ships as component-only, currently unused but reserved for future composition). User feedback: low-World aliasing character interacts nicely with Mulch — worth exploring more.
- [ ] **XYZ engine** — cryptic 3-parameter reverb (X size/texture morph, Y saturate+undersample coupled-on-one-knob, Z meta-routing with three regimes: Nested predictable / Folded signature / Coupled chaotic). The signature sound is the per-pass aliasing of generated harmonics in Folded mode. Coupled mode MUST inherit RotCoat's reduced-rate trick on AM335x or it overruns budget. Design doc: `planning/xyz-engine-design.md`. Package: `house`. Lands after RotCoat.

### Reverb design philosophy

- [-] Reusable primitives + combination mechanics distilled from the handoff into `planning/reverb-design-philosophy.md`. Covers Householder FDN, allpass bank, delay tuning ratios, feedforward-with-one-feedback, one-pole damper as the structural primitives; plus 5 combination mechanics (Console wrapper as feedback governor, undersample/Bezier as character axis, topology morph, shared matrix with two reads, cross-modulated feedback). Cross-references existing habitat units (kWoodRoom, Network) and the queued ports. Read this before designing any new reverb to avoid reinventing.

### House package atom architecture

- [-] **Structural pattern locked**: `house` package is a library of reusable C++ atoms (`od::Object` subclasses, each one a thin DSP node) + optional C++ harnesses (for patterns that need internal feedback or shared state) + Lua unit compositions. Mirrors the firmware's "small Objects + Lua-composed Units" pattern. Iteration loop = swap atoms in a Lua chain, rebuild, listen. Architecture doc: `planning/house-atom-architecture.md`. Affects everything else in this package going forward.
- [ ] **kWoodRoom Phase 1 refactor**: promote `KWoodRoomDSP` (currently a plain DSP helper) to a proper `od::Object` subclass at `mods/house/atoms/KWoodRoom.h`. Inlets / outlets / parameters / process(). Then `mods/house/assets/KWoodRoom.lua` becomes a thin GainBias-driven unit that just wires the atom up. Keeps the per-sample DSP body verbatim; only the wrapper changes. Per the atom architecture doc.
- [ ] **AW tone-shaping atoms — first-15 starter port** (`planning/airwindows-primitives-inventory.md`). Port these atoms as individual `od::Object` subclasses in `mods/house/atoms/`, one header each: Density, Spiral, Drive, Mojo, Slew, Acceleration, Creature, Capacitor2, Baxandall, Console0 Channel+Buss, DeRez2, ChromeOxide, Point, Swell, Fracture. Each gets a Smoketest-gated first-run on hardware before being wired into a real unit. Once the atom library covers ~15 entries, start composing chains as units (Saturate, Drive, Lo-Fi, etc.) — the unit IS the chain, swap atoms to iterate on character. Tone-shapers are FLOP-bound (not memory-bound like reverbs), so chains are cheap on float+NEON. Latency-aware harness rule: keep Bite (~2 smp), ToneSlant (block), and any averaging filter out of tight feedback returns.
- [ ] **Channel-Buss containment harness** (`planning/airwindows-primitives-inventory.md` §5). First harness pattern worth lifting from Lua to C++: wraps an inner chain in a Console0Channel → ... → Console0Buss pair, providing the level-dependent anti-runaway containment that MV/MV2 uses to ship infinite-decay reverbs without screech. Implement as either a Lua template (preferred — simpler, more inspectable) OR a C++ harness Object in `mods/house/harnesses/ChannelBussPair.h` if the wiring overhead matters. Lua-first; lift only if listening proves the overhead. This is mechanic #1 in `planning/reverb-design-philosophy.md`.
- [ ] **Envelope-driver harness pattern** — Swell / Surge / Point / Flutter / Capacitor2's FM as drivers that modulate parameters of an inner chain. Likely Lua-side wiring via ParameterAdapter (their `Out` outlets drive other atoms' parameter inlets). Codify the pattern once the first such chain ships.
- [ ] **Reduced-rate domain harness** — for RotCoat and any other unit needing host→reduced→host with Bezier reconstruction at the boundary. C++ harness Object (the rate boundary needs internal scheduling that Lua can't express). Defer until RotCoat implementation starts.
...
- [ ] **Faust reverb library examples** -- attempt to port one as a follow-up route for the Network cascade-FDN ambition we abandoned at `2.6.2.0` (see `planning/network-cascade-postmortem.md` for the lessons from the retrofit attempt). Candidates: `fverb`, `jot_rev`, `zita_rev1` in the Faust libraries (GRAME / various BSD/MIT) -- generate C++ via faust compiler then integrate. Starts from a known-working FDN topology rather than retrofitting onto a multi-tap cascade. The Network unit's plexus viz can be layered onto either (visualization can lie about the underlying topology). Goal: a working "nasty verb" we can iterate on for sound design, not a math exercise. **CloudSeedCore was attempted 2026-05-17/18 (commits `2190131` Phase A vendor + `6ed6308` Phase B wind-down, preserved on branch `cloudseed-archive`) and abandoned**: `ReverbController::Process` first-frame hangs the Cortex-A8 audio thread regardless of compiler-opt level, alignment, or input data. Construction was clean (SD-trace confirmed) but DSP never returned. Do not retry CloudSeedCore without first root-causing the upstream Cortex-A8 hang.
...
- [ ] **Visadhara drum voice** (clean-room from public BIA manual). Six tonal + one noise oscillator in three configurations (Skin / Liquid / Metal); Skin = 6-osc additive, Liquid = additive + pitch envelope, Metal = pair of 3-op PMM. Threshold-reflection folder with dynamic stage multiplication. Reuses jf::four::Voice from JF Phase 3a (NEON 4-lane pattern), Helicase polynomial sine, Varishape morph. Not the same lineage as Ngoma (analog-808/909) -- Visadhara is digital-aliasing-friendly. **Architecture-from-manual reference: `planning/bia-clone-scoping.md`. Phase-by-phase implementation plan: `planning/visadhara-initial-pass.md`** (5 phases: Skin skeleton → folder+attack+noise → Liquid → Metal → polish+ship). Ships in spreadsheet alongside Ngoma + JF.
