...
- [ ] **Airwindows port (2): WoodenBox.** Replaces Galactic2 as the next pick per handoff: filed under AW's "Tone Color" not Reverb, miniature DI-to-acoustic verb, dense and confined, **smaller/cheaper than a full k-verb and lowest-risk port in the catalog**. Package: `house`. Per kWoodRoom's CloudSeed-trap discipline, run a Smoketest harness gate before binding to a real unit. Source path: `airwindows/airwindows/Plugins/MacVST/WoodenBox/source/*.cpp,*.h`. Convert per-sample `double` work to `float` per our AM335x convention.
