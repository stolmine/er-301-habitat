...
- [x] Zipper noise on V/Oct and comb size changes. *(2.5.5.184. Doppler-style baseDelay smoother per Path B in `planning/pecto-zipper.md`. New members: `od::LinearRamp mFade` (25ms ramp at frame rate, matches `od::Delay` setLength), `mPrevBaseDelay`, `mCurBaseDelay`. Block-rate snap-on-`mFade.done()` chains overlapping smooth transitions during continuous knob motion. Per-sample `currentBase = fade[i]*prev + (1-fade[i])*cur` interpolates baseDelay scalar; Pass A's NEON inner loop multiplies `s.tapPosition[t]` by `vdupq_n_f32(currentBase)` instead of reading a cached delay array. Continuous Doppler glide during sweeps -- musically right for a multitap comb. .182/.183 attempted SDK-faithful dual-pipeline crossfade and crashed Cortex-A8 on doubled NEON register pressure; reverted (see `272c968`/`334406d`) and pivoted to Path B. Tier 2 hint count actually DROPPED vs `.181` baseline (16 -> 10 total, process() 3 -> 1) because the new layout removed `mCachedDelaySamples` and replaced its block-rate auto-vec'd recompute with explicit-NEON inline math.)*
