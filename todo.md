...
- [x] Sub-display selection indicator lands on the wrong sub-button after ply-to-ply navigation. Fixed 2026-04-21 as Decision 8 (see planning/shift-handling.md). In each Pattern A control's `onCursorEnter`, after delegating to `GainBias.onCursorEnter`, call `self:setSubCursorController(nil)` guarded by `if self.paramMode` (`~= 0` on BandControl's 3-mode cycle). Clears the GainBias-inherited `self.bias` highlight so no sub is highlighted on paramMode entry until the user deliberately taps one, matching Decision 7's convention. Applied to 21 Pattern A controls; Pattern C untouched (EncoderControl doesn't bind `self.bias` at init).
