...
- [ ] Sub-display selection indicator desyncs on ply-to-ply navigation. Discovered on Helicase (2026-04-21) during shift-handling smoke test. Repro: focus a sub readout on ply A (e.g. sub1 on HelicaseOverviewControl), then press M key to move to ply B (another subparam control, e.g. HelicaseModControl). On ply B, the sub-display visual selection indicator lands on sub3 (or some spurious position), but the encoder still routes to the originally-focused readout as if ply A were still active. Encoder behavior is "correct" per the current wiring; the indicator is what lies. Likely root cause candidates: (a) `setSubCursorController(nil)` in onCursorLeave doesn't clear the per-SubButton visual highlight -- only the Widget-level cursor controller ref; (b) the new ply's first render after onCursorEnter uses stale `paramFocusedReadout` from a prior session before we cleared it (unlikely given our leave-side clear, but worth checking the order vs addSubGraphic); (c) the sub-button highlight is driven by a global encoder-focus hook that wasn't repointed when the cursor hopped between plies. Audit every Pattern A and Pattern C control for this behavior across adjacent ply transitions; likely fix is to explicitly clear the sub-button highlight state (not just `paramFocusedReadout`) on leave, or to assert that `onCursorEnter` always drives the visual indicator to "none" before any focus restoration. Also verify that the *encoder* really is staying with the old readout vs. routing to the new ply's default -- if it is, that's a separate (worse) bug and needs a fix on how `releaseFocus` / `grabFocus` interacts with the encoder's subCursorController slot.
