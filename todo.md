...
- [ ] **Move Alembic from spreadsheet to catchall.** Alembic at .188 is interesting but not where it needs to be for a spreadsheet ship -- promotion to catchall puts it alongside the other "experimental / WIP" voices (Flakes, Lambda, Sfera, Som) so it gets release exposure without anchoring spreadsheet. Touches: AlembicVoice.{cpp,h}, AlembicSphereGraphic.{h,cpp}, all assets/Alembic*Control.lua + assets/AlembicVoice.lua, the `%include` lines in spreadsheet.cpp.swig (remove) + catchall.cpp.swig (add). Also moves the Alembic comb retrofit todo (Doppler smoother + idx wrap ulp guard + isfinite IIR reset, see Alembic section above) into catchall scope. Bump catchall to 0.3.0; spreadsheet drops Alembic accordingly. Verify Sample.Pool integration still works under catchall's package context.
- [ ] **Consolidate Mutable Instruments ports into one package.** Currently 8 separate packages (clouds, commotio, grids, marbles, plaits, rings, stratos, warps), some with cross-package deps (e.g. Stratos uses the Clouds reverb engine). Cross-package deps mean users hit "missing package" errors if they install partially -- and there's no real reason to ship them as separate units. Plan: collapse all 8 into a single new package whose name follows the no-third-party-branding convention (per `feedback_no_third_party_branding` -- candidates: "ports", "library", "mi", or just functional like "synthesis"; locking the name is a sub-decision). Touches: merge mod.mk OBJECTS lists, merge `.cpp.swig` files (concat the unit `%include`s + `#include`s into one wrapper source), merge assets/ trees, retire the 8 individual `mods/<name>/` dirs (or keep source there but build into the consolidated package). Bump the new consolidated package to 1.0.0 (it's a fresh public package). Test matrix: insert each of the 9 units (Plaits, Clouds, Rings, Grids, Warps, Stratos, Commotio, Marbles T, Marbles X) and verify all work, especially the cross-references (Stratos→Clouds reverb, anything else that shared resources). Watch for symbol collisions if multiple units have static helpers with the same name across the original packages.
