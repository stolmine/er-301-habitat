...
- [ ] Sub-display selection indicator lands on the wrong sub-button after ply-to-ply navigation (2026-04-21, Helicase). **Clarified repro:** focus sub N on top-level control 1 (e.g. sub1 = mixReadout on HelicaseOverviewControl). Press M key to move cursor to top-level control 2 (HelicaseModControl). On control 2, encoder DOES correctly edit control 2's sub N -- i.e. the sub-index carries over as desired. **But** the sub-display's selection highlight on control 2 is drawn under sub3, not sub N. So the encoder routing is right; only the visual is wrong. The bug is therefore about sub-button highlight state, not focus state. Root cause candidates: (a) SubButton's pressed/active highlight has its own per-widget state that survives across `addSubGraphic` / `removeSubGraphic` cycles and isn't touched by `setSubCursorController(nil)`; (b) the framework's encoder-focus indicator on control 2 is reading a stale "last known sub" from somewhere -- possibly a global or per-window encoder-focus field that wasn't updated when cursor hopped; (c) sub-index inheritance across plies is a framework-level feature (it IS happening per the clarification above), but the visual handoff is busted by an off-by-N or stale-frame bug in how the highlight is rendered. Audit path: instrument SubButton draw/highlight state on both plies, trace what the framework sets the indicator position to during the M-key cursor move, and compare Pattern A controls with stock GainBias-multi-sub controls (DriveControl paramMode vs a Pattern B like BandListControl) to see whether the stock behavior matches. Check every Pattern A + Pattern C control for this once the framework-side cause is understood; the fix is likely in how our `setParamFocusedReadout` helper (or our `setSubCursorController` call) updates the visual selection, not in any per-control Lua.
