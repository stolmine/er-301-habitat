...
- [ ] **Airwindows port (3): CreamCoat.** Bright-ambience engine whose mechanic (undersample → Bezier reconstruct, sane regime) is the exact pattern kWoodRoom already implements via kWoodRoom's outer Bezier loop. CreamCoat is the canonical reference for the divisor+Bezier character axis. Package: `house`. Smoketest gate.
