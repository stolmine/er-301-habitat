...
- [ ] SOM filter pair mapping options (depends on the bolt-on above). Som structure: 2 TPT SVFs per channel with shared Q, mux-based 8-slot routing matrix, parallel↔cascade morph (`topoMix`), per-filter FM via differential dest accumulator, BP/LP bridge blend, drive/exciter. Reference: `mods/catchall/Som.cpp:588-685`, `mods/catchall/Som.h:24-36`. Per-node trainable slots that open up: 2 cutoffs, shared Q, `topoMix`, `bpLpBlend`, 2 filter-FM amounts, `drive` (≈8 floats). Either expand row from 29 to ≈37, or repurpose `reagent[28]` region. Mapping driver candidates: brightness → cutoff, pitched → resonance, flux → drive, runLen → topoMix (stationary samples cascade, transient samples parallel).
- [ ] Meta-mapping targets — non-audio / aggregate-statistic features that should shape global mapping topology rather than per-node values. Deferred from Phase 5c, captured here so the design space stays visible: (a) per-sample feature variance bounds the per-node `chaosScore` range (low-variance samples cluster all 64 nodes around 1-2 topologies; high-variance samples span the table); (b) mean entropy across picks biases topology distribution (globally tonal samples skew CHAIN/PARALLEL even when individual picks have high local chaos; globally noisy samples skew DENSE/SELF_FB); (c) sample length adjusts coarse hop or pick density; (d) stereo correlation drives detune asymmetry (decorrelated → larger detunes, correlated → tighter); (e) DC offset / waveform asymmetry drives matrix-path bias direction.
- [ ] Serial stacked waveform viz for the 64 trained wavetable frames (Phase 5d artifact). Once Phase 5d ships per-node sample-window-derived transfer-function LUTs, render the 64 frames as a stacked / cascading waveform display — perspective stack down the Z axis with the active scan-position frames lit, neighbors dimmed, and Hamilton-chain ordering visible at a glance. Could replace or supplement the sphere viz, or live as a per-unit "wavetable inspector" view. Source data: `mWavetableLUT[64][32]`.
