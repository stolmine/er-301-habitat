...
- [x] Timebase and gain controls (scope dev 1.2.0.1). Header-only `ScopeGraphic.h` package-side, aped from firmware MiniScope, exposes `setDecimation` (uses public `FifoProbe::setDecimation`) and `setGain` (Y-axis scale in `calculate()`). Shared `ScopeView.lua` ViewControl with sub-display Time (7 stops, 1x..64x) + Gain (5 stops, 0.25x..4x) selectors driven by M1/M2 focus + encoder stepping. All three variants (Scope, Scope 2x, Scope Stereo) share the same view; stereo applies time/gain to both L and R graphics. State persists via Unit serialize/deserialize. No firmware changes. Plan: planning/scope-timebase-gain.md. **Fix (2026-05-29)**: graphics weren't rendering on hardware — root cause was an `else if (mCalculateCount >= 0)` "cleanup" of MiniScope's warmup counter that broke the increment invariant (counter stuck at -10, draw early-returned forever). Reverted to MiniScope's verbatim `else { mCalculateCount++; }`. See `feedback_identical_means_identical.md`. Version bumped to 1.2.0.1 (rather than .1.0.3) so `install-packages.sh`'s version-sort picks this build over the stale `scope-1.2.0.pkg` still present in `testing/am335x/` from before the dev-version rollback.
- [ ] Research headerless unit display (SDK hardcodes header — not currently possible)
- [x] Voltmeter readout on existing scope units (scope dev 1.2.0.3). Sub-display S3 column shows a 3-decimal rolling-mean voltage scaled to ER-301 ±10 V range, useful for DC and V/Oct analysis. Header-only `ScopeVoltsReadout` graphic follows the scope's `ScopeGraphic` instance and reads `getVolts()` (computed from the full probe buffer mean × 10) on each draw. Integration window scales with timebase: ~0.33 s at decimation=2, ~10 s at 64. Right-justified text so the decimal column stays put as digits change. (Original todo proposed a separate voltmeter unit — folded into the existing scope variants instead since the probe + buffer infrastructure was already there.)
