...
- [-] **Airwindows port queue** (revised 2026-06-02 after handoff integration — see `planning/refs/airwindows-port-handoff.md` + `planning/airwindows-reverb-research.md` addendum). Active port: **Cabinet** (kWoodRoom). Then in order: WoodenBox → CreamCoat → BrightAmbience3 → Galactic → Verbity. All land in the new `house` package.
- [-] **Airwindows port (1): Cabinet (kWoodRoom).** Active. Phase A (house package scaffold) shipped at `house-0.1.0.1`; Phase 0 (CabinetDSP + Smoketest harness) shipped at `house-0.1.0.2`. **Awaiting hardware smoke-test pass before Phase 1.** Plan: `planning/kwoodroom-port-plan.md`.
- [ ] **Airwindows port (2): WoodenBox.** Replaces Galactic2 as the next pick per handoff: filed under AW's "Tone Color" not Reverb, miniature DI-to-acoustic verb, dense and confined, **smaller/cheaper than a full k-verb and lowest-risk port in the catalog**. Package: `house`. Per Cabinet's CloudSeed-trap discipline, run a Smoketest harness gate before binding to a real unit. Source path: `airwindows/airwindows/Plugins/MacVST/WoodenBox/source/*.cpp,*.h`. Convert per-sample `double` work to `float` per our AM335x convention.
- [ ] **Airwindows port (3): CreamCoat.** Bright-ambience engine whose mechanic (undersample → Bezier reconstruct, sane regime) is the exact pattern Cabinet already implements via kWoodRoom's outer Bezier loop. CreamCoat is the canonical reference for the divisor+Bezier character axis. Package: `house`. Smoketest gate.
