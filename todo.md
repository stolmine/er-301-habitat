...
- [ ] **Distortion becomes audible as mix is brought down at high drive** (user report 2026-06-15, drive 5.2, all three bands at unity, no morph, no comp/output offsets, type=Tube/Diode/Tri Fold all reproduce). User initially thought "internal hard clipping when being mixed" but the mix line itself (`MultibandSaturator.cpp:791`, `out = dry + (wet - dry) * mix`) is a clean lerp — at mix=1 the heavily-saturated wet masks artifacts, at mix<1 the dry pulls overall level down and unmasks them. Two likely culprits, both in the wet path:
  ...
  - **(b) Discontinuous safety clamp** at `MultibandSaturator.cpp:750`: `if (wet > 1.5f || wet < -1.5f) wet = fast_tanh(wet * 0.67f) * 1.5f;`. Conditional branch creates a derivative discontinuity at ±1.5 which itself generates harmonics that alias. Replace with a smooth continuous limiter (e.g. `wet = wet / pow(1 + pow(|wet|/1.5, p), 1/p)` style pseudo-saturate, or just always-on `fast_tanh(wet * 0.67f) * 1.5f` without the threshold).
