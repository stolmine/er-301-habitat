...
- [ ] **CloudSeed / Faust reverb library examples** -- attempt to port one of these as a follow-up route for the Network cascade-FDN ambition we abandoned at `2.6.2.0` (see `planning/network-cascade-postmortem.md` for the lessons from the retrofit attempt). Two candidates: (1) **CloudSeed** by ValdemarOrn (BSD-3-Clause, https://github.com/ValdemarOrn/CloudSeed) -- C# / C++ algorithmic reverb modeled after Valhalla designs, uses early-reflection diffusers + late-tail FDN topology with per-line damping; the structural choices we'd need are already made and audited. (2) **Faust reverb examples** (`fverb`, `jot_rev`, `zita_rev1` in the Faust libraries, GRAME / various BSD/MIT) -- generate C++ via faust compiler then integrate. Both routes start from a known-working FDN topology rather than retrofitting onto a multi-tap cascade. The Network unit's plexus viz can be layered onto either one (visualization can lie about the underlying topology). Goal: a working "nasty verb" we can iterate on for sound design, not a math exercise.
