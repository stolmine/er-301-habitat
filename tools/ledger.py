#!/usr/bin/env python3
"""er-301-habitat ledger tool — the BDG traceability machine.
...
gate). Code references an item as a [hab:<id>] comment tag at its impl seam.

Ported from er-301-stolmine's tools/ledger.py per
planning/ledger-regime-porting-301.md §3 (er-301-habitat). Habitat is a
package repo: all code lives under mods/<pkg>/ (C++ DSP + Lua middle layer),
packages already namespace themselves in item-id prefixes (pecto-*, excel-*),
so areas are concern buckets rather than per-package buckets.
"""
...
# today's paths.
LEDGER = Path(os.environ.get("HAB_LEDGER") or (ROOT / "planning" / "ledger.toml"))
TODO = Path(os.environ.get("HAB_TODO") or (ROOT / "planning" / "TODO.md"))
CLAIMS = Path(os.environ.get("HAB_CLAIMS") or (ROOT / "planning" / "claims.toml"))
# Package source trees, repo-relative. All habitat code lives under mods/<pkg>/.
# NEVER add `testing/` here — in 301 repos it is build output (generated object
# files), not source or tests; scan() also skips it defensively.
SRC_DIRS = [ROOT / "mods"]
...
# real harness lands. The intended future substrate is the stolmine headless emu
# (package repos run inside it) — see planning/ledger-regime-porting-301.md §2.3.
TEST_DIRS = [ROOT / "tests"]

STATUSES = {"todo", "wip", "done", "blocked"}
AREAS = {"dsp", "units", "ui", "seq", "tooling", "docs", "infra"}
TAG_RE = re.compile(r"\[hab:([a-z0-9][a-z0-9-]*)\]")
# Placeholder: no test harness exists, so this never matches anything today.
...
def collect_tags():
    """id -> [relpaths] for every [hab:<id>] tag in the source + test trees."""
    tags = {}
...
def collect_testcases():
    ...
    # (the stolmine headless emu is the planned substrate).
    names = set()
...
def validate(items):
    ...
    # orphan tags: a [hab:x] in code with no matching item
    for tag_id, locs in tags.items():
        if tag_id not in seen:
            errors.append(f"orphan tag [hab:{tag_id}] (no such item) in {locs[0]}")
    ...
    for it in items:
        ...
        # anchor: an item flagged anchor=true must carry a [hab:id] tag in src.
        if it.get("anchor") and iid not in tags:
            errors.append(f"{iid}: DONE anchor=true but no [hab:{iid}] tag found in src/")
...
AREA_TITLES = {
    "dsp": "DSP", "units": "Units", "ui": "UI / interaction",
    "seq": "Sequencing / timing", "tooling": "Tooling / build",
    "docs": "Documentation", "infra": "Infrastructure",
}
STATUS_MARK = {"done": "✓", "wip": "~", "todo": " ", "blocked": "✗"}
AREA_ORDER = ["dsp", "units", "ui", "seq", "tooling", "docs", "infra"]


def render_text(items):
    lines = []
    lines.append("ER-301-HABITAT TODO (generated — DO NOT EDIT)")
    lines.append("============================================")
    lines.append("")
...
def cmd_render(_args):
    ...
    try:
        ...
        shown = TODO.name  # HAB_TODO points outside the repo (a self-test copy)
