...
local MiniSequencer = Class {}
MiniSequencer:include(Unit)

function MiniSequencer:init(args)
  args.title = "Mini Seq"
  args.mnemonic = "MS"
  Unit.init(self, args)
end

function MiniSequencer:onLoadGraph(channelCount)
  local op = self:addObject("op", libpeaks.MiniSequencer())
...
function MiniSequencer:onLoadViews()
  return {
    gate = Gate {
      button      = "gate",
      description = "Gate",
    ...
    p2 = GainBias {
      ...
      range = self.objects.p2, biasMap = Encoder.getMap("[-1,1]"),
      biasPrecision = 2, initialBias = 0.0
    },
    p3 = GainBias {
