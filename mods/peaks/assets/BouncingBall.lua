...
local BouncingBall = Class {}
BouncingBall:include(Unit)

function BouncingBall:init(args)
  args.title = "Bouncing Ball"
  args.mnemonic = "BB"
  Unit.init(self, args)
end

function BouncingBall:onLoadGraph(channelCount)
  local op = self:addObject("op", libpeaks.BouncingBall())
...
function BouncingBall:onLoadViews()
  return {
    gate = Gate {
      button      = "gate",
      description = "Gate",
    ...
    p1 = GainBias {
      button = "grav", description = "Gravity",
