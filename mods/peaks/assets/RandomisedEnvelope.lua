...
local RandomisedEnvelope = Class {}
RandomisedEnvelope:include(Unit)

function RandomisedEnvelope:init(args)
  args.title = "Rand Envelope"
  args.mnemonic = "RE"
  Unit.init(self, args)
end

function RandomisedEnvelope:onLoadGraph(channelCount)
  local op = self:addObject("op", libpeaks.RandomisedEnvelope())
...
function RandomisedEnvelope:onLoadViews()
  return {
    gate = Gate {
      button      = "gate",
      description = "Gate",
