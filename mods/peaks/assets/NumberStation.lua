...
local NumberStation = Class {}
NumberStation:include(Unit)

function NumberStation:init(args)
  args.title = "Number Station"
  args.mnemonic = "NS"
  Unit.init(self, args)
end

function NumberStation:onLoadGraph(channelCount)
  local op = self:addObject("op", libpeaks.NumberStation())
...
function NumberStation:onLoadViews()
  return {
    gate = Gate {
      button      = "gate",
      description = "Gate",
