...
local SnareDrum = Class {}
SnareDrum:include(Unit)

function SnareDrum:init(args)
  args.title = "Snare Drum"
  args.mnemonic = "SD"
  Unit.init(self, args)
end

function SnareDrum:onLoadGraph(channelCount)
  local op = self:addObject("op", libpeaks.SnareDrum())
...
function SnareDrum:onLoadViews()
  return {
    gate = Gate {
      button      = "gate",
      description = "Gate",
    ...
    p1 = GainBias {
      button = "freq", description = "Frequency",
