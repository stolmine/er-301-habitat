...
local ByteBeats = Class {}
ByteBeats:include(Unit)

function ByteBeats:init(args)
  args.title = "ByteBeats"
  args.mnemonic = "BB"
  Unit.init(self, args)
end

function ByteBeats:onLoadGraph(channelCount)
  local op = self:addObject("op", libpeaks.ByteBeats())
...
function ByteBeats:onLoadViews()
  return {
    gate = Gate {
      button      = "gate",
      description = "Gate",
    ...
    p1 = GainBias {
      button = "form", description = "Formula",
