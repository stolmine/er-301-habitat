...
#include "DrumVoice.h"
#include "FilterResponseGraphic.h"  // for lutSin / lutCos (package-trig safe)
#include <math.h>
#include <stdint.h>
...
namespace stolmine
{
  ...
  // ABI-hardening: full implementation moved INLINE into the header so the
  // class has NO key function and the vtable is COMDAT-emitted in every TU
  // that uses it (matches HelicaseOrbitalGraphic / ColmatageOverviewGraphic /
  // every other working spreadsheet graphic).
  //
  // Background: when DrumCubeGraphic::draw was defined out-of-line in the
  // .cpp, draw became GCC's "key function" and the vtable was emitted ONLY
  // in DrumCubeGraphic.o. After firmware rebuilds the cube would hard-fault
  // on insert (specifically on od::Graphic::addChild call paths that touch
  // the child's vtable). All other spreadsheet graphics inline their
  // virtual draw() — none of them faulted. Moving the cube to match.
  ...
  public:
    ...
    virtual void draw(od::FrameBuffer &fb)
    {
      ...
      for (int i = 0; i < 8; i++) {
        vx[i] = kVerts[i][0]; vy[i] = kVerts[i][1]; vz[i] = kVerts[i][2];
      }
      ...
      for (int i = 0; i < 8; i++) {
        ...
        if (len > 0.0001f) {
          ...
          vz[i] = z + (sz - z) * sphereT;
          if (foldT > 0.0f) {
            float wave = lutSin(len * 6.0f + mAngleY * 3.0f);
            float disp = foldT * 0.25f * wave;
            vx[i] += nx * disp; vy[i] += ny * disp; vz[i] += nz * disp;
          }
        }
      }
      ...
      float cosY = lutCos(mAngleY), sinY = lutSin(mAngleY);
      float cosX = lutCos(mAngleX), sinX = lutSin(mAngleX);
      ...
      for (int fi = 0; fi < visCount; fi++) {
        ...
        fillQuad(fb, faceGray, dotting, f, gritNoise,
                 px[i0], py[i0], px[i1], py[i1], px[i2], py[i2], px[i3], py[i3]);
