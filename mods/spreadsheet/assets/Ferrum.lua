...
-- Built from the profiled Modbap Trinity NEON/FM algorithm
-- (~/repos/trinity-midi-harness/findings.md). Emergent 2-op FM: quantized ratio,
-- index->feedback, grit->noise, pitch env (sweep depth / time rate), hold+decay amp env.
local app = app
...
function Ferrum:onLoadGraph(channelCount)
  local op = self:addObject("op", libspreadsheet.Ferrum())
