...
local DelayInfoControl = Class {
  type = "DelayInfoControl",
  canEdit = false,
  canMove = true
}
DelayInfoControl:include(Base)

function DelayInfoControl:init(args)
  local delay = args.delay or app.logError("%s.init: delay is missing.", self)

  Base.init(self, "overview")
  self:setClassName("DelayInfoControl")

  local width = args.width or ply

  local graphic = app.Graphic(0, 0, width, 64)
  self.pDisplay = libstolmine.DelayInfoGraphic(0, 0, width, 64)
  ...
  -- Sub-display: grain / taps / stack readouts
...
function DelayInfoControl:zeroPressed()
  if self.focusedReadout then
...
function DelayInfoControl:cancelReleased(shifted)
  if self.focusedReadout then
