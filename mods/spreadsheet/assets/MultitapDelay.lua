...
local FeedbackControl = require "spreadsheet.FeedbackControl"
local TapListControl = require "spreadsheet.TapListControl"
...
function MultitapDelay:onLoadGraph(channelCount)
  ...
  -- Drift
  local drift = self:addObject("drift", app.ParameterAdapter())
  drift:hardSet("Bias", 0.0)
  tieParam("Drift", drift)
  ...
  tieParam("Reverse", reverse)
  self:addMonoBranch("reverse", reverse, "In", reverse, "Out")

  -- Stack
  local stack = self:addObject("stack", app.ParameterAdapter())
  stack:hardSet("Bias", 0)
...
function MultitapDelay:onLoadViews()
  return {
    ...
    overview = DelayInfoControl {
      ...
      stack = self.objects.stack:getParameter("Bias")
    },
