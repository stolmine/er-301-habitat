...
local HelicaseSyncControl = Class {
  type = "HelicaseSyncControl",
  canEdit = false,
  canMove = true
}
HelicaseSyncControl:include(Base)

function HelicaseSyncControl:init(args)
  local button = args.button or "sync"
  local description = args.description or "Sync"
  ...
  local phaseThresholdParam = args.phaseThresholdParam or app.logError("%s.init: phaseThresholdParam is missing.", self)

  Base.init(self, button)
  self:setClassName("HelicaseSyncControl")
  ...
  -- Phase threshold readout (replaces voltage threshold)
  self.threshold = app.Readout(0, 0, ply, 10)
  self.threshold:setParameter(phaseThresholdParam)
  self.threshold:setAttributes(app.unitNone, threshMap)
  self.threshold:setPrecision(2)
...
function HelicaseSyncControl:onRemove()
  self.branch:unsubscribe("contentChanged", self)
  Base.onRemove(self)
end

function HelicaseSyncControl:contentChanged(chain)
...
function HelicaseSyncControl:setFocusedReadout(readout)
  if readout then readout:save() end
  self.focusedReadout = readout
  self:setSubCursorController(readout)
end

function HelicaseSyncControl:zeroPressed()
  if self.focusedReadout then self.focusedReadout:zero() end
  return true
end

function HelicaseSyncControl:cancelReleased(shifted)
  if self.focusedReadout then self.focusedReadout:restore() end
  return true
end

function HelicaseSyncControl:spotReleased(spot, shifted)
...
function HelicaseSyncControl:enterReleased()
...
function HelicaseSyncControl:subPressed(i, shifted)
...
function HelicaseSyncControl:subReleased(i, shifted)
  if shifted then return false end
  if i == 1 then
    if self.branch then
      self:unfocus()
  ...
  elseif i == 2 then
    if self:hasFocus("encoder") then
      if self.focusedReadout == self.threshold then
        ...
        local kb = Decimal {
          message = "Phase threshold for sync receptivity.",
          commitMessage = "Phase threshold updated.",
...
function HelicaseSyncControl:encoder(change, shifted)
...
function HelicaseSyncControl:upReleased(shifted)
  if self.focusedReadout then
    self.focusedReadout = nil
