...
function TimeControl:subReleased(i, shifted)
  if self.paramMode then
    local readout, label
    if i == 1 then readout, label = self.gridReadout, "grid"
    elseif i == 2 then readout, label = self.reverseReadout, "reverse"
    elseif i == 3 then readout, label = self.skewReadout, "skew"
    end
    if readout then
      if shifted then
