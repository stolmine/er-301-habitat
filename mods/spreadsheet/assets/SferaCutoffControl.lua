...
local SferaCutoffControl = Class {}
SferaCutoffControl:include(GainBias)

function SferaCutoffControl:init(args)
  ...
  local desc = app.Label("Q Scale", 10)
  desc:fitToText(3)
  ...
  self.paramSubGraphic:addChild(app.SubButton("Q", 1))
end

function SferaCutoffControl:setParamMode(enabled)
...
function SferaCutoffControl:onCursorEnter(spot)
  GainBias.onCursorEnter(self, spot)
  self:grabFocus("shiftPressed", "shiftReleased")
end

function SferaCutoffControl:onCursorLeave(spot)
...
function SferaCutoffControl:shiftPressed()
  self.shiftHeld = true
  self.shiftUsed = false
...
function SferaCutoffControl:shiftReleased()
...
function SferaCutoffControl:spotReleased(spot, shifted)
  if self.paramMode then
    self.paramFocusedReadout = nil
    self:setSubCursorController(nil)
...
function SferaCutoffControl:subReleased(i, shifted)
  if shifted then return false end
  if self.paramMode then
    if i == 1 and self.qReadout then
      self.qReadout:save()
      self.paramFocusedReadout = self.qReadout
      self:setSubCursorController(self.qReadout)
...
function SferaCutoffControl:encoder(change, shifted)
...
function SferaCutoffControl:zeroPressed()
  if self.paramMode and self.paramFocusedReadout then
...
function SferaCutoffControl:cancelReleased(shifted)
  if self.paramMode and self.paramFocusedReadout then
