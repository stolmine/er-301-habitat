...
function BandControl:subReleased(i, shifted)
  if self.paramMode == 1 then
    local readout, label
    if i == 1 then readout, label = self.amtReadout, "amount"
    elseif i == 2 then readout, label = self.biasReadout, "bias"
  ...
  elseif self.paramMode == 2 then
    local readout, label
    if i == 1 then readout, label = self.weightReadout, "weight"
