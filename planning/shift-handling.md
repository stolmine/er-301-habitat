...
| 8 | Suppress inherited sub3 highlight in paramMode, default-highlight bias-bound sub | **Partial -- PINNED 2026-04-22.** On `onCursorEnter`, if `self.paramMode` is active, call `self:setSubCursorController(self.paramModeDefaultSub)`. GainBias.init points the stock sub-cursor-controller at `self.bias` (sub3 position in stock sub-display); that reference persists and renders a spurious highlight at sub3 when `paramSubGraphic` is swapped in. On paramMode entry we replace it with the author-declared `paramModeDefaultSub` -- nil on controls with no bias-bound sub (equivalent to an explicit clear), or the relevant sub-readout on the five controls where sub1 shares the Bias parameter with `self.bias`. Applied to all 21 Pattern A controls; Pattern C untouched. BandControl's 3-mode variant uses `paramMode ~= 0` as the guard. Five `paramModeDefaultSub` declarations: HelicaseOverviewControl (mixReadout), HelicaseModControl (ratioReadout), HelicaseShapingControl (indexReadout), LaretOverviewControl (skewReadout), MixInputControl (levelReadout via fallback). **Status:** spurious sub3 highlight is confirmed cleared (good). But the five bias-bound controls still show no visible sub-highlight in paramMode after the refinement -- the encoder still edits bias, sub1's readout still ticks visually (shared Parameter), but no highlight frame renders on sub1. Possible causes captured in the `todo.md` Shift audit follow-up entry. Encoder routing is unchanged throughout and is correct. |
...
| **Colmatage** | ColmatageBlockControl, ColmatageRepeatsControl, ColmatageTextureControl, MixControl | F(all fader); T on all four; K on BlockControl(phrase min/max, block max), Repeats(ritard/blend/accel), Texture(amp min/max, fade), Mix(input/output/tanh). BSP tile-field overview viz unaffected. | **pass 2026-04-21** |
...
| **Pecto** (moved to spreadsheet, previously biome) | MixControl, TransformGateControl, DensityControl (plus stock GainBias / Pitch plies: size, feedback, inputLevel, outputLevel, tanhAmt, tune) | Biggest shared-control consumer. F across all three; T; K on Mix(input/output/tanh), Density(pattern/slope/resonator); xform ply same as Excel. NEON 3-pass gather path unchanged — CPU should still sit at ~6% stereo. | **pass 2026-04-21** (initially failed K + P due to stale device-side biome-2.1.0; passed after install. Moved to spreadsheet in same session to eliminate cross-package require for DensityControl's ShiftHelpers dep.) |

### biome package

(Pecto has been moved to the spreadsheet package.) No affected controls remain in biome after the Pecto move.
