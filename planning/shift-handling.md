...
| 8 | Suppress inherited sub3 highlight in paramMode, default-highlight bias-bound sub | **On `onCursorEnter`, if `self.paramMode` is active, call `self:setSubCursorController(self.paramModeDefaultSub)`.** (1) GainBias.init points the stock sub-cursor-controller at `self.bias` (sub3 position in stock sub-display); that reference persists and renders a spurious highlight at the sub3 position of whatever subGraphic is currently attached when paramMode swaps in `paramSubGraphic`. On paramMode entry we replace it with the author-declared `paramModeDefaultSub` — nil on controls with no bias-bound sub (equivalent to the original clear), or the relevant sub-readout on controls where sub1 shares the Bias parameter with `self.bias` so the user actually sees what the encoder edits. (2) Applies to all 21 Pattern A controls; Pattern C untouched (EncoderControl doesn't init `subCursorController` to any readout). BandControl's 3-mode variant uses `paramMode ~= 0` as the guard. (3) Five controls have sub1 bias-bound and declare `paramModeDefaultSub`: HelicaseOverviewControl (mixReadout), HelicaseModControl (ratioReadout), HelicaseShapingControl (indexReadout), LaretOverviewControl (skewReadout), MixInputControl (levelReadout via fallback). The other 16 leave the field nil; behavior matches the original clear. (4) Encoder routing is unchanged — governed by `focusedReadout` / `paramFocusedReadout` at `GainBias.lua:786-793`. `paramFocusedReadout` stays nil on entry (Decision 7 convention); the default highlight is purely visual and reflects the bias edit path. |
