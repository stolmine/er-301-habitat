...
| 5 | 6 | Sample-slot wiring + difference-based sampling + audio-domain feature extraction + Layer 0/1 O0+O1 + post-PMM colorization (wavetable + filter pair + routing matrix + comb) + Ferment macro. First end-to-end train-on-load pass + complete DSP parameter space lock-in + first user-control macro. **Shipped in seven sub-phases**: 5a = sample slot plumbing; 5b = analysis kernel (coarse-pass features → farthest-point pick of 64 moments → Layer 0 + Layer 1 O0+O1 → populate per-instance mPresetTable); 5c = topology selection + spectral flatness + detune[8]; 5d-1 = wavetable transfer-function "reagent" (per-node 256-entry sample-window-derived waveshaper LUTs with peak-fold + per-node even/odd symmetry + feature-driven wavefolder; preset row pre-grown 29 → 43 floats; independent reagent scan position); 5d-2 = Som-style filter pair (2 TPT SVFs in series with topoMix / bpLpBlend / shared Q / drive, lifted from `mods/catchall/Som.cpp:623-654`; 6 base trained slots row[29..34]); 5d-3 = full mux routing matrix (10 sources × 6 destinations × 8 lanes per node, training-driven (src, dst, atten) selection so filter-FM from PMM and filter↔filter cross-mod fall out naturally; sparse 10×6 matrix precollapsed at block setup, ~20 fmacs per sample); 5d-4 = comb (Pecto cut-down: 16-tap multitap delay + 4 patterns + 4 resonator types + feedback, 5 trained slots row[43..47], single-fader collapsed dry/wet+comb-scan ply, third independent scan position joining main + reagent) + Ferment chaos scalar (single-axis macro on matrix + routing attens, default 1.0 = trained, 0 = clean tonal voice). Reuses pffft, Som's feature pattern, Som's TPT SVF + fastTan / fastExp, and Pecto's bufRead/bufWrite/bufReadInterp + resonator filter primitives (`mods/spreadsheet/Pecto.cpp`). After 5d-4, the visible UI has 8 plies (tune/freq/sync/scan/reagent/comb/ferment/level); per-region user-bias fades remain a Phase 7+ option if the Ferment-only macro proves insufficient. |
