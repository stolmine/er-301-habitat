...
| 5 | 6 | Sample-slot wiring + difference-based sampling + audio-domain feature extraction + Layer 0/1 O0+O1. First end-to-end train-on-load pass. **Shipped in two sub-phases**: 5a = sample slot plumbing (audio-only file picker via Sample.Pool conventions, claim/release lifecycle, menu Tasks for Card/Pool/Detach/Edit, serialize via SamplePool helpers, per-instance preset table relocation); 5b = analysis kernel (coarse-pass features → farthest-point pick of 64 moments → Layer 0 + Layer 1 O0+O1 → populate per-instance mPresetTable). Reuses pffft (`mods/spreadsheet/pffft.h`) and Som's RMS/ZCR/brightness/flux pattern (`mods/catchall/Som.cpp:332-379`). |
