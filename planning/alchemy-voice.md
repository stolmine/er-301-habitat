...
| 3 | 3–4 | 64-slot × 29-float preset table + K-weighted block-rate crossfader. Fork Som skeleton, strip training params. First custom paramMode controls land here. **Shipped in two sub-phases**: 3a = C++ scan/crossfader + stock GainBias scan ply, drops the 29 Phase 2 user-direct adapter wirings from the Lua UI (Parameters stay on the C++ class for Phase 7's group-fades hookup); 3b = custom `AlembicScanControl` with canonical paramMode shift-toggle, K (path window) on the sub-display. |
| 4 | 5 | Sphere viz, Path A (4×4 tiles, partition cache, time slicing, rotation LUT). **Shipped in two sub-phases**: 4a = architecture + basic sphere (tile-granular Voronoi, partition cache keyed on `(rotBucketX, rotBucketY, scanNode)`, time-sliced recompute, pre-baked sin/cos LUTs, scan-following camera integrator, depth shading); 4b = aesthetic refinements (lifted focused node, slow Z-roll drift, scan-proximity per-node brightness modulation). 2 inline getters added to AlembicVoice for the viz: `getScanNode()` and `getNodeBrightness(int n)`. |
| 5 | 6 | Sample-slot wiring + difference-based sampling + audio-domain feature extraction + Layer 0/1 O0+O1 + post-PMM colorization (wavetable + filter pair + routing matrix). First end-to-end train-on-load pass + complete DSP parameter space lock-in. **Shipped in six sub-phases**: 5a = sample slot plumbing (audio-only file picker via Sample.Pool conventions, claim/release lifecycle, menu Tasks for Card/Pool/Detach/Edit, serialize via SamplePool helpers, per-instance preset table relocation); 5b = analysis kernel (coarse-pass features → farthest-point pick of 64 moments → Layer 0 + Layer 1 O0+O1 → populate per-instance mPresetTable); 5c = topology selection + spectral flatness + detune[8] (categorical matrix-routing templates picked per node by chaosScore signature, layered over 5b's continuous strength mapping; fills the 7 unused matrix paths with discrete-state emergence; flatness joins as feature #7 from existing FFT; op-A detune driven by flatness × runLen); 5d-1 = wavetable transfer-function "reagent" (per-node 32-entry sample-window-derived waveshaper LUTs replacing the planned scalar nonlinearity; preset row pre-grown 29 → 43 floats); 5d-2 = Som-style filter pair (2 TPT SVFs in series with topoMix / bpLpBlend / shared Q / drive, lifted from `mods/catchall/Som.cpp:623-654`; 6 base trained slots row[29..34]); 5d-3 = full mux routing matrix (10 sources × 6 destinations × 8 lanes per node, training-driven (src, dst, atten) selection so filter-FM from PMM and filter↔filter cross-mod fall out naturally; sparse 10×6 matrix precollapsed at block setup, ~20 fmacs per sample). Reuses pffft (`mods/spreadsheet/pffft.h`), Som's RMS/ZCR/brightness/flux pattern (`mods/catchall/Som.cpp:332-379`), and Som's TPT SVF + fastTan / fastExp (`mods/catchall/Som.cpp:31-67, 588-685`). After 5d-3, all per-node trainable dimensions (43 floats + 16 uint8) and user-bias Parameters (36 hooks) are locked; Phase 7 designs ≈9 plies (5-6 group fades + 3 globals) on top of this stable inventory. |
