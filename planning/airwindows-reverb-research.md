...
source reading. It vindicates the Cabinet (kWoodRoom) pick
...
- **Moderate (modern engines, ~one instance per AM335x)** — k-series (Cabinet is here), Galactic family, Verbity, NonlinearSpace, CloudCoat, Chamber/Chamber2, ClearCoat/CreamCoat/CrunchCoat, BrightAmbience3
...
- **double → float + NEON is mandatory.** ARMv7 NEON is single-precision SIMD only; every `double` op falls back to slow scalar VFPv3. This is the cost of admission for any AW port. Cabinet currently ships at full double per the conservative Phase-0 plan — Phase 1+ converts the 3×3 trellis to float first, then evaluates 6×6 conversion via listening test.
...
- **Reduced internal rate is the primary headroom lever** on AM335x. CreamCoat's mechanic (which Cabinet already uses via the outer Bezier) is structurally AM335x-friendly: it cuts buffer length AND read rate proportionally to the divisor.
- **Denormal floor in all feedback paths.** Cabinet's denormal flush already in place at the per-sample input boundary; preserve as we port more.

### Action items inserted into the queue

The post-Cabinet port order is updated in
`planning/kwoodroom-port-plan.md` and reflected in `todo.md` Port
Candidates entries. The condensed plan:

1. **Cabinet** (kWoodRoom port) — in progress, Phase 0 shipped
2. **WoodenBox** (new pick replacing Galactic2 as next) — lowest-risk port, small wooden tone-shaper
3. **CreamCoat** — proves the divisor+Bezier pattern in isolation; Cabinet validates it but CreamCoat is the canonical implementation
