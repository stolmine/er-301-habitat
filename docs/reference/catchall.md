...
| — (visualizer) | display ply | — | — | The ferrofluid sphere. Not editable. |
...
| — (visualizer) | display ply | — | — | Draws the currently morphed 256-sample wavetable frame; brightness follows output level. |
...
**I/O** — A **generator**: it has no audio input. The chain input is connected to a
...
| `Feedback` (`fdbk`) | GainBias | 0 … 1 | 0.0 | Blends the voice's own output into what the feature analyzer listens to. At 1 the map trains purely on itself. This is an analysis-path feedback — it is not an audio feedback loop through the filters. |
...
back into the sample itself. You don't dial operators; you **scan** that chain of 64
states. Reagent scans a second, independent position for the wavefolding curve,
...
| `Sync` (`sync`) | Gate (trigger mode) | trigger | — | A rising edge hard-resets all four operator phases to zero. |
...
**Menu** — a **Sample Menu**: `Select from Card`, `Select from Pool`,
