...
number station, and a bytebeat engine. All units are **mono, generator-only** —
...
|---|---|---|---|---|
| Clock | Gate | gate mode | — | Tempo input. Each rising edge feeds the period predictor. |
| Reset | Gate | trigger mode | — | Resets the oscillator phase to the Phase offset. |
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| Gate | Gate | gate mode | — | Triggers the drum on the rising edge. |
...
length. Trigger it and it does one thing well.

**Controls**

| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| Gate | Gate | gate mode | — | Triggers the hat on the rising edge. |
...
Great for accelerating retrigger ramps and natural-feeling decays.

**Controls**

| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| Gate | Gate | gate mode | — | Drops the ball (rising edge resets position and velocity). |
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| Clock | Gate | gate mode | — | Advances to the next step on each rising edge. |
| Reset | Gate | trigger mode | — | Arms a reset — the sequence jumps to step 1 on the *next* clock edge. |
...
starts a transmission. Excellent as an atmospheric noise source rather than a
tuned instrument.
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| Gate | Gate | gate mode | — | Starts/holds a transmission. |
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| Gate | Gate | gate mode | — | Fires the envelope on the rising edge. |
...
steps 5–8 are their **inversions**. You get a symmetric, self-balancing
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| Clock | Gate | gate mode | — | Advances one step per rising edge (8 steps). |
| Reset | Gate | trigger mode | — | Arms a reset — jumps to step 1 on the next clock edge. |
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| Reset | Gate | trigger mode | — | Resets both the main and the modulator phase. |
...
overdrive, triangle skew — giving a wave that morphs as it cycles. Distinct
timbral movement without any pitch drift.
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| Clock | Gate | gate mode | — | Reference clock. Its period sets the base frequency (with tempo prediction). |
| Reset | Gate | trigger mode | — | Resets the oscillator phase to zero. |
| Pitch | GainBias | `[0,1]` | 0.50 | Clock multiplier, continuous and piecewise-log: 0.0 = /16, 0.5 = ×1, 1.0 = ×16. |
| Shape | GainBias | `[0,1]`, 7 presets | 0.50 | Waveshape preset, in sevenths — same seven slots as WSM LFO: 1 Folded sine · 2 Folded power sine · 3 Folded power sine · 4 Overdriven sine · 5 Triangle · 6 Square · 7 Square. |
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| Gate | Gate | gate mode | — | Rising edge resets `t` and the phase to 0, restarting the pattern. |
...
- High Hat exposes only a Gate. peaks::HighHat::Configure takes four params
  ...
  0.5f → 32768. Notably that leaves the two randomness terms at ~50%, so hits
...
- README's Peaks table lists all 14 units and the categories match toc.lua.
  ...
  * "Mini Sequencer — 4-step CV sequencer" is right, but README's
    "Mod Sequencer — Extended step sequencer" undersells/obscures that it is
    specifically 8 steps where the last four are inversions of the first four.
  * "PLO — Phase-locked oscillator" is accurate; worth noting it is the only
    unit here intended for audio-rate use and it takes its pitch from a clock,
    not V/Oct.
...
- Actual audible ranges (rate in Hz, decay in ms, etc.) — these come from
  lookup tables in resources.cc (lut_lfo_increments, lut_env_increments,
