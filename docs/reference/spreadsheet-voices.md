...
|---|---|---|---|---|
| `Steps` | step-list display + editor | 64 steps | — | The pattern. Turn to scroll the step cursor; press a sub button to edit that step's field. |
| `Sequence` | info display | — | — | Overview of length / loop / transform scope. Enter expands it. |
| `Clock` | Gate | — | — | Advances the sequence on each rising edge. |
| `Reset` | Gate | — | — | Rising edge jumps to step 1 and clears the tick counter. Cuts nothing else. |
...
| `Transform` | custom Gate + params | — | — | Fires the transform on a rising edge. Shift toggles the sub-display between gate and parameter pages. |
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| `Steps` | chaselight display + editor | 64 steps | — | The pattern. Turn to scroll; sub buttons edit the focused step. |
| `Sequence` | info display | — | — | Length / loop / gate width overview. Enter expands. |
| `Clock` | Gate | — | — | Advances the sequence and measures the clock period (used for gate length and ratchet spacing). |
| `Reset` | Gate | — | — | Jumps to step 1, clears the tick counter, and cuts any sounding gate. |
| `Ratchet` | custom Gate + params | — | — | Held high at a step's gate-fire moment, that step retriggers. Shift toggles gate/parameter sub-pages. |
| `Transform` | custom Gate + params | — | — | Fires the pattern transform on a rising edge. |
| `Seq Length` | GainBias (expansion) | 1 … 64 | 16 | Number of active steps. |
...
| 4 | `inv` | Inverts every gate. | — | — |
| 5 | `rot` | Rotates the pattern right. | Rotation amount | — |
| 6 | `den` | Per-step coin flip at the given density. | Density percent (0 … 100) | — |
...
| `Segments` | segment-list display + editor | up to 32 | — | The breakpoint list. Turn to scroll; sub buttons edit the focused segment. |
| (curve display) | 2-ply transfer-curve graphic | — | — | Draws the current transfer function with the live input/output dot. Enter expands. |
...
| `Transform` | custom Gate + params | — | — | Fires the shape transform on a rising edge. |
...
| 1 | `rot` | Rotates offset/curve/weight left. Depth selects the rotation *distance*, not strength — always full-strength. |
...
| (viz) | phase-space display | — | — | Read-only 3-D delay embedding of the output, slowly rotating with phosphor persistence. Resets when the algorithm changes. |
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| `V/Oct` | Pitch | — | — | 1 V per octave into the carrier. |
...
| `Sync` | custom Gate | — | — | Phase-receptive hard sync on the carrier. |
| `Level` | GainBias | 0 … 1 | 0.5 | Output gain. |
...
| 2 | abs-sine | Rectified — octave-doubling. |
...
**Menu** — *Mode*: `Quality` — `lo-fi` / `hi-fi`, default `lo-fi`. Hi-fi turns
on 2× oversampling across the whole chain, enables fractional morphing between
adjacent carrier and modulator shapes, and drops the lo-fi ~8-bit quantization
of the folded carrier and modulator. Lo-fi's grit is a feature, not just a
saving.
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| `Trigger` | Gate (trigger mode) | — | — | Fires the voice. The whole mode bank is baked on the rising edge, so V/Oct is sampled per hit — mid-hit pitch changes do not bend it. |
| `V/Oct` | custom Pitch | — | — | 1 V per octave. Base pitch is 110 Hz; the result is clamped to 8 Hz … 8 kHz. |
...
octaves. `Character`: `shape`, `grit`, `punch` ("Shape / Grit / Punch") — this
page is shown by default. `Sweep`: `time` ("Sweep Time"). `Decay`: `hold`, `atk`
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| `V/Oct` | Pitch | — | — | 1 V per octave on top of TIME. |
...
| `Trigger 1N (IDENTITY)` … `Trigger 6N` | Gate ×6 (gates view) | — | — | Per-voice trigger inlets. |
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| `Trigger` | Gate | — | — | Fires the envelope on all voices and resets every phase for a coherent attack peak. |
| `V/Oct` | custom Pitch | — | — | 1 V per octave. Base pitch is 110 Hz. |
...
| `Harmonic` | GainBias (Mode expansion) | 0 … 1 | 0.5 | Trades distribution against detune. At 0 all eight voices sit on the fundamental with full detune — a fat chorused sub. At 1 they spread onto the Spread series with detune collapsed to zero. |
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| `V/oct` | Pitch | — | — | 1 V per octave on the modulator, which is what you hear as pitch. |
...
6. Visadhara's Mode is fully implemented, not a stub. Visadhara.lua:148 still
   comments "Phase 1 stub — Skin only; Phase 3+ wires the crossfade between
   modes". The C++ implements Skin, Liquid and Metal with both tent-crossfade
   and snap dispatch. Stale comment; not repeated in the docs above.
