...
| Response | FilterResponseControl (radial response display) | n/a | n/a | Live composite response / per-band energy display. Its sub-display carries Band Count, V/Oct Offset and Slew. |
...
- Bands: `Randomize bands` sets a random frequency 60 Hz-16 kHz and gain 0-3 per band.
- Bands: `Rescan .scl files` re-reads Scala files from `front/scales` into the custom scale slots.
- Macro Filter Type: `All peaking`, `All lowpass`, `All resonator`. Each sets every band's type at once.
...
| Grain / Taps / Stack | DelayInfoControl (overview display) | n/a | n/a | Live tap/energy overview. Its sub-display carries Grain Size, Tap Count and Stack. |
...
| Skew | GainBias (expansion) | -2…2 | 0.00 | Exponent on tap spacing (2^skew), bunching taps toward the start or the end of the span. |
...
A three-band saturator. The signal is split by two 24 dB/oct Linkwitz-Riley crossovers whose positions you set with per-band Weight and a global Skew, and each band gets its own waveshaper, DC bias, level and morphing state-variable filter. A shared Drive with tilt-EQ Tone feeds all three, and the output stage adds an optional compressor and tanh limiter. It works for adding weight to the lows without fizzing the highs, and for outright destruction at high Drive.
...
A three-band compressor with an FFT spectrum display: the incoming signal is driven, optionally tilted, split into LO/MID/HI, compressed per band, and recombined against a dry/wet mix. The two crossover points are placed by the per-band Weight balance and then pushed around by a single bipolar **Skew** control, so you steer where the bands sit rather than dialling exact hertz. There is a dedicated sidechain input that can be switched in to key all three detectors, and each band's spectrum ply shows its live gain-reduction contour.
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| Clock | LaretClockControl (comparator + branch) | trigger | n/a | Advances the sequencer. Also measures the clock period, which sets the loop length for stutter / reverse / shuffle / pitch. |
...
| Transform | TransformGateControl (gate + branch) | gate | n/a | On a rising gate (or the `fire!` sub-button) applies the selected transform to the step program. |
...
- **Input 1 / 2 / 3**: default sub-display shows the patched source mnemonic plus **Solo** and **Mute** panels (all three inputs are enrolled in the unit mute group, and solo/mute state is saved with the patch). Shift swaps to `level` / `wght` / `ofst` readouts. Expanding gives the input ply plus **Level N** (0 - 2, default 1.00), **Weight N** (0 - 2, default 1.00; it scales that bell's width relative to Focus) and **Offset N** (0 - 1; defaults 0.17 / 0.50 / 0.83 for inputs 1/2/3, the bell's centre on the scan axis).
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| Clock | Gate (trigger) + sub-display | n/a | n/a | Clock input. One trigger per beat; the unit divides each beat into `Subdivision` units. |
...
|---|---|---|---|---|---|
| Clock | Reset | Gate (trigger) | n/a | n/a | Restarts the phrase. |
...
| `sitr` | Sitar: amplitude-following modulation of the write position, giving jawari buzz and pitch wobble that follow playing level. |
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| (overview ply, button `in`) | Custom overview | n/a | n/a | Routing view for the three per-band inputs; see below. |
...
| V/oct | Pitch | n/a | n/a | 1 V/oct transposition of the whole band group (1 V = 12 semitones). |
...
**Sub-display / expanded**: The `in` ply is a routing overview: a three-stripe graphic (LOW / CENTRE / HIGH) showing each band's post-routing input, with an "ALL" overlay on any band currently falling back to the main input. Its sub-display holds three MiniScopes, one per band input; sub-buttons 1/2/3 open the corresponding LOW / CENTRE / HIGH subchain. There are no level or bias controls there: the per-band inputs pass at unity, so any shaping happens inside the subchain you open.
...
| Control | Type | Range / Options | Default | What it does |
|---|---|---|---|---|
| Size | Custom overview (GainBias + fabric viz) | 0 to 1 | 0.35 | Room size, scaling the tank delay lengths. Main dial of the unit; its fader is replaced by the fabric waterfall visualization. |
...
| Cutoff B | GainBias | 0 to 1 | 0.50 | Core B cutoff, same mapping; this is the interference/comb partner. Detuning A against B is where the comb structure comes from. |
...
- Vitrail cutoff Hz range (~30 Hz to ~5 kHz, saturating near k = 0.8) is derived
  from clockHz().
- Canals V/oct has no initialBias; recorded as `n/a`.
