...
Manual QA checklist for each custom unit in the habitat packages. Runnable pre-release against a fresh emulator (and optionally hardware) to catch regressions. Testing round 1 on firmware v0.7.0-stolmine.9.0.0.
...
tested 2026-04-12 v2.1.0

**Module**: `TrackerSeq` &middot; **Mnemonic**: `Ex` &middot; **Category**: `Sequencer` &middot; **Stereo**: `native-stereo`

#### 1. Insert / load
- [p] Inserts without error.
- [p] Default state: 16-step sequence, each step offset 0, length 1, deviation 0; clock triggers advance; reset resets to step 0.
- [p] Hot-plug behavior: sequence resumes from current step without discontinuity.

#### 2. Top-level row (expanded view)
Order: `steps`, `info`, `clock`, `reset`, `slew`, `xform`.
- [p] **steps** (StepListControl) -- 64-step grid display with current-step editing; rotate: step highlight moves, paging when at boundaries.
- [p] **info** (SeqInfoControl) -- visual sequence info readout; rotate: cycle focus between seqLength, loopLength, xformScope sub-controls.
- [p] **clock** (Gate) -- clock input jack + comparison threshold; rotate: adjust threshold level.
- [p] **reset** (Gate) -- reset input jack; rotate: adjust threshold.
- [p] **slew** (GainBias) -- CV-controlled slew time, range [0, 10] s; rotate: adjust slew time.
- [p] **xform** (TransformGateControl) -- transform/snapshot gate + function selector + factor dial; rotate: cycle through 9 functions (add, sub, mul, div, mod, reverse, rotate, invert, random) or adjust factor.

#### 3. Sub-displays
- [p] **steps** sub-display shows three readouts: `offset` (EditOffset param, mapped ±5V or ±1V), `length` (EditLength, 1-16 ticks), `dev` (EditDeviation, 0-1). Each SubButton "offset", "length", "dev" focuses that readout for numeric entry.
- [p] **info** sub-display shows `length` (seqLength 1-64), `loop` (loopLength 0-64, 0=disabled), `scope` (xformScope: ofst/len/dev/all). Each SubButton "length", "loop", "scope" focuses that readout.
- [p] **xform** sub-display shows gate threshold readout, mini waveform scope, function label, and factor readout; SubButtons "input", "thresh", "fire" toggle sub-display mode between gate view and math fire.

#### 4. Expansion views
- [p] **info** focus -> `seqLen`, `loopLen`, `xformScope` readout controls.
- [ ] **xform** focus -> function label, factor dial, mode toggle. - missing on all spreadsheet xform controls, undecided on CV input here

#### 5. Menu (onShowMenu)
- [p] **Set All Step Lengths** -- tasks "1 tick", "2 ticks", "4 ticks".
- [f] **Offset Range** -- "10Vpp (-5 to +5)" and "2Vpp (-1 to +1)"; re-maps readout. - does not remap live, user needs to change again
- [p] **Offsets** -- "Randomize all offsets", "Clear all offsets".
- [p] **Snapshot** -- "Save snapshot" / "Restore snapshot" (all 64 triplets).

#### 6. CV input behavior
- [p] clock rising edge advances by 1 step.
- [p] reset rising edge returns to step 0.
- [p] slew 0-10 s smooth on continuous CV.
- [p] seqLength / loopLength / xformScope CV modulates over ranges. - overview exp params pass

#### 7. Stereo routing - N/A
...
- [p] Quicksave + reload: all 64 (offset, length, deviation) tuples, offsetRange10v flag, xformFunc/Factor/Scope restored.
- [p] Two back-to-back cycles identical.
- [p] `loadStep(0)` called on deserialize so edit buffer matches step 0.

#### 9. Edge cases
- [f] Seq length 1 + no loop: clock triggers same step repeatedly. - it's possible for sequence playhead to get stuck in limbo outside of step sequence if we change length while playhead is outside of new bounds. for instance, we are looking at overview ply reading out 14/1 for play position, output is odd, seems like we might actually still be looping outside of nominal bounds
- [p] Loop length < seq length: playback wraps at loop boundary, then continues.
- [p] Loop length > step length: playback wraps at step boundary, then continues.
- [ ] Divide-by-zero / mod-0 in transform factor handled gracefully. - there is no 0 option in xform factor range
- [p] Offset range change mid-sequence: readout remaps without touching stored values.
- [ ] Max step length + max slew: clock period must exceed slew time or output queues. - not sure what this means
...
#### 1. Insert / load
- [p] Inserts without error.
- [p] Default: 16-step sequence, gates OFF, lengths 1 tick, velocity 1.0.
- [p] Hot-plug: gate state persists, resumes at current step.

#### 2. Top-level row (expanded view)
Order: `steps`, `info`, `clock`, `reset`, `ratchet`, `xform`.
- [p] **steps** (ChaselightControl) -- chaselight grid of lit/unlit dots.
- [p] **info** (GateSeqInfoControl) -- rotate cycles focus through seqLength, loopLength, gateWidth.
- [p] **clock** (Gate) -- clock input + threshold.
- [p] **reset** (Gate) -- reset input + threshold.
- [p] **ratchet** (RatchetControl) -- ratchet gate + multiplier + RatchetLen/RatchetVel toggle options.
- [p] **xform** (TransformGateControl) -- function selector + paramA/paramB (euclidean, NR, grids, necklace, invert, rotate, density).

#### 3. Sub-displays
- [p] **steps** sub-display: gate ON/OFF toggle, length readout (1-16 ticks), velocity (0-1). SubButtons "on/off", "length", "vel".
- [p] **info** sub-display: seqLength, loopLength, gateWidth readouts. SubButtons "length", "loop", "width".
- [p] **ratchet** sub-display: multiplier readout plus RatchetLen and RatchetVel BinaryIndicator toggles. Shift+SubButton toggles each option.
- [p] **xform** sub-display: paramA, paramB, function label, gate scope. SubButtons "input", "paramA", "paramB".

#### 4. Expansion views
- [p] **info** focus -> seqLen, loopLen, gateWidthFader.

#### 5. Menu (onShowMenu)
- [p] **Set All Gate Lengths** -- "1 tick", "2 ticks", "4 ticks".
- [p] **Set All Velocities** -- "25%", "50%", "100%".
- [p] **Randomize** -- gates / lengths / velocities.
- [p] **Clear / Reset** -- gates / lengths / velocities to defaults.
- [p] **Snapshot** -- Save / Restore.

#### 6. CV input behavior
- [p] clock rising edge advances step.
- [p] reset rising edge returns to step 0.
- [p] ratchet gate splits current step into N sub-gates.
- [p] gateWidth CV modulates duty cycle.
- [ ] xformScope CV selects gate/length/velocity/all. - only operates on gate positions in Ballot by design

#### 7. Stereo routing - N/A, cv sequencer
- [ ] Mono chain: single gate output.
- [ ] Stereo chain: duplicated to L/R.
- [ ] RatchetLen / RatchetVel options affect both channels.

#### 8. Save / load round-trip
- [f] Quicksave + reload: all 64 (gate, length, velocity) tuples, ratchetLen/Vel options, xformFunc/paramA/paramB/scope restored. - ratchet settings do not persist
- [ ] Two back-to-back cycles identical. - precluded by above
- [f] **Known-fragile**: RatchetLen and RatchetVel options have regressed in the past -- confirm both survive a full cycle.

#### 9. Edge cases
- [p] All gates OFF: ratchet on a zero step produces no output.
- [p] Velocity 0 + ratchet 8: sub-gates are zero amplitude.
- [p] Loop length 1: only step 0 plays.
- [f] Gate width 0: no pulse -- verify graceful handling. - we get a trig, which mihgt be the preferable behavior
- [p] Euclidean with 0 hits: no gates fire.
- [p] RatchetLen toggle + gate length 1: sub-gates compressed into 1-tick window.
